...
bool CCrypto::Encrypt(CByteArray& inData, CByteArray& arData,int ll)
{
	...
	//	Before we write to the file, trash it.
	m_file.SetLength(0);
	m_file.Write(inData.GetData(),inData.GetCount());
...
bool CCrypto::Decrypt(const CByteArray& arData, CByteArray& outData)
{
	//	Return failure if we don't have a context or key.
	if(m_hCryptProv == NULL || m_hKey == NULL)
		return false;
	...
	BYTE* buffer=new BYTE[m_file.GetLength()];
	m_file.Read(buffer,m_file.GetLength());

	for(int i=0;i!=m_file.GetLength();i++)
	{
