...
namespace OOAdvantech.WindowsAzureTablesPersistenceRunTime
{
    ...
    public class ObjectUnderTransactionDataManager
    {
        ...
        TypeDictionary TypeDictionary
        {
            get
            {
                return DataLoader.TypeDictionary;
        ...
        public void PrepareTablesWithDataOfObjectsUnderTransaction()
        {
            if (DataLoader.UpdatedObjects.Count > 0 || DataLoader.NewObjects.Count > 0 || DataLoader.CandidateForDeleteObjects.Count > 0)
            {
                ...
                foreach (DataLoader.DataColumn column in DataLoader.ClassifierDataColumns)
                    _TableWithStatesOfObjectsUnderTransaction.Columns.Add(column.Name, column.Type);
                    ...
                    if (subNodeDataLoader != null && !dataNode.ThroughRelationTable)
                    {
                        ...
                        if (subNodeDataLoader.HasRelationReferenceColumns)
                        {
                            ...
                            foreach (PersistenceLayerRunTime.ObjectsLink relationChange in DataLoader.GetRelationChanges(dataNode.AssignedMetaObject as DotNetMetaDataRepository.AssociationEnd, dataNode))
                            {
                                ...
                                {
                                    MetaDataRepository.ObjectIdentityType referenceObjectIdentityType = referenceObjectIdentityTypes[referenceObjectIdentityTypes.IndexOf((relationChange.RoleA.RealStorageInstanceRef.ObjectID as ObjectID).ObjectIdentityType)];
                                    MetaDataRepository.ObjectIdentityType objectIdentityType = DataLoader.ObjectIdentityTypes[referenceObjectIdentityTypes.IndexOf((relationChange.RoleA.RealStorageInstanceRef.ObjectID as ObjectID).ObjectIdentityType)];
                                ...
                                else
                                {
                                    ...
                                    MetaDataRepository.ObjectIdentityType referenceObjectIdentityType = referenceObjectIdentityTypes[referenceObjectIdentityTypes.IndexOf((relationChange.RoleB.RealStorageInstanceRef.PersistentObjectID as ObjectID).ObjectIdentityType)];
                                    MetaDataRepository.ObjectIdentityType objectIdentityType = DataLoader.ObjectIdentityTypes[referenceObjectIdentityTypes.IndexOf((relationChange.RoleB.RealStorageInstanceRef.PersistentObjectID as ObjectID).ObjectIdentityType)];
                                    foreach (MetaDataRepository.IIdentityPart part in objectIdentityType.Parts)
                                    {
                                        foreach (MetaDataRepository.IIdentityPart referencePart in referenceObjectIdentityType.Parts)
                                        {
                        ...
                        else
                        {
                            ...
                            foreach (PersistenceLayerRunTime.ObjectsLink relationChange in DataLoader.GetRelationChanges(dataNode.AssignedMetaObject as DotNetMetaDataRepository.AssociationEnd, dataNode))
                            {
                                ...
                                {
                                    MetaDataRepository.ObjectIdentityType referenceObjectIdentityType = referenceObjectIdentityTypes[referenceObjectIdentityTypes.IndexOf((relationChange.RoleA.RealStorageInstanceRef.ObjectID as ObjectID).ObjectIdentityType)];
                                    MetaDataRepository.ObjectIdentityType objectIdentityType = subNodeDataLoader.ObjectIdentityTypes[subNodeDataLoader.ObjectIdentityTypes.IndexOf((relationChange.RoleA.RealStorageInstanceRef.ObjectID as ObjectID).ObjectIdentityType)];
                                ...
                                else
                                {
                                    ...
                                    MetaDataRepository.ObjectIdentityType referenceObjectIdentityType = referenceObjectIdentityTypes[referenceObjectIdentityTypes.IndexOf((relationChange.RoleB.RealStorageInstanceRef.ObjectID as ObjectID).ObjectIdentityType)];
                                    MetaDataRepository.ObjectIdentityType objectIdentityType = DataLoader.ObjectIdentityTypes[referenceObjectIdentityTypes.IndexOf((relationChange.RoleB.RealStorageInstanceRef.ObjectID as ObjectID).ObjectIdentityType)];
       ...
       MetaDataRepository.ObjectQueryLanguage.IDataTable _TableWithStatesOfObjectsUnderTransaction;
        ...
        void LoadTableWithRelationChangesInTransaction(OOAdvantech.MetaDataRepository.ObjectQueryLanguage.DataNode relatedDataNode)
        {
           ...
           OOAdvantech.MetaDataRepository.ObjectQueryLanguage.IDataTable associationTable = null;
            ...
            {
                foreach (MetaDataRepository.IIdentityPart part in objectIdentityType.Parts)
                    associationTable.Columns.Add(part.Name, part.Type);
