...
namespace OOAdvantech.WindowsAzureTablesPersistenceRunTime.AzureTableMetaDataPersistenceRunTime
{
    ...
    public class ClassBLOB
    {
        ...
        /// <MetaDataID>{05c7d7b2-ca16-45b3-b51e-3a1e5b5afc14}</MetaDataID>
        internal ClassBLOBData ClassBLOBData;




        /// <MetaDataID>{DF2CF68A-A910-454F-B96E-8CAD58101AA8}</MetaDataID>
        public ClassBLOB(DotNetMetaDataRepository.Class _class,ClassBLOBData classBLOBData)
        {
            ClassBLOBData = classBLOBData;
        ...
        /// <MetaDataID>{8bb6c71a-e1e4-43b2-81a9-0a0fc7acc3e8}</MetaDataID>
        byte[] ByteStream;
        /// <MetaDataID>{fb4cdef5-6464-4c53-a03d-f9abb0db21b5}</MetaDataID>
        int MembersOffset;
        ...
        public ClassBLOB(byte[] byteStream, int offset)
        {
            ...
            DotNetMetaDataRepository.Assembly _assembly = DotNetMetaDataRepository.Assembly.GetComponent(assembly) as DotNetMetaDataRepository.Assembly;
            System.Type type = assembly.GetType(typeFullName);

            //OOAdvantech.DotNetMetaDataRepository.MetaObjectMapper.FindMetaObjectFor(assembly) as DotNetMetaDataRepository.Assembly;
            //if (_assembly == null)
            //{
            //    using (Transactions.SystemStateTransition stateTransition = new Transactions.SystemStateTransition())
            //    {
            //        _assembly = new OOAdvantech.DotNetMetaDataRepository.Assembly(assembly);
