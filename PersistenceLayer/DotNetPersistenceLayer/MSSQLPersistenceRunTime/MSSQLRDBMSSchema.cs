...
{
    /// <MetaDataID>{44f73f92-e2d0-4464-a15f-e3af28765d99}</MetaDataID>
    public class MSSQLRDBMSSchema : OOAdvantech.RDBMSDataObjects.IRDBMSSQLScriptGenarator
    {
       ...
       public bool SupportAddRemoveKeys { get { return true; } }
        ...
        public MSSQLRDBMSSchema(DataBase dataBase)
        {
            DataBase = dataBase;
        }
        ...
        public System.Collections.Generic.List<ForeignKeyData> RetrieveDatabaseForeignKeys(Table table)
        {
            ...
            while (dataReader.Read())
            {
                string foreignKeyName = dataReader["FK_NAME"] as string;
                ForeignKeyData fkeyData = null;
                if (!foreignKeysData.TryGetValue(foreignKeyName, out fkeyData))
                {
                    fkeyData = new ForeignKeyData((string)dataReader["FK_NAME"], (string)dataReader["FK_TABLE_NAME"], (string)dataReader["REF_TABLE_NAME"]);
                    foreignKeysData[foreignKeyName] = fkeyData;
                }
                fkeyData.ColumnsNames.Add((string)dataReader["COLUMN_NAME"]);
                fkeyData.ReferedColumnsNames.Add((string)dataReader["REF_COLUMN_NAME"]);
            }
