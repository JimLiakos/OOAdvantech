	...
	public class Column : OOAdvantech.MetaDataRepository.MetaObject
	{
		...
		public override OOAdvantech.MetaDataRepository.MetaObjectID Identity
		{
			get
			{
				...
				if(_Identity==null)
					_Identity=new MetaDataRepository.MetaObjectID(Namespace.Name+"."+Name);
				return _Identity;
			}
		...
		public override void Synchronize(MetaDataRepository.MetaObject OriginMetaObject)
		{
			...
			if(Length>0)
				Datatype=TypeDictionary.GetDBType(ColumnDotNetType);
			else
			{
				Datatype=TypeDictionary.GetDBType(ColumnDotNetType,false);
				Length=TypeDictionary.GeDefaultLength(ColumnDotNetType);
			}
            if (!TypeDictionary.IsTypeVarLength(ColumnDotNetType))
                Length = 0;
		...
		internal string DataBaseColumnName;
		internal string DataBaseColumnDataType;
		internal int DataBaseColumnLength;
		internal bool DataBaseColumnAllowNulls;
		internal bool DataBaseColumnIdentityColumn;
		...
		internal bool HasChangeColumnFormat
		{
			get
			{
