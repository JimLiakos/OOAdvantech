	...
	public class UnlinkAllObjectCommand : PersistenceLayerRunTime.Commands.OnMemoryUnlinkAllObjectCommand
	{
		...
		public override void GetSubCommands(int currentExecutionOrder)
		{
			...
			if(!SubTransactionCmdsProduced)
			{
				...
				RDBMSPersistenceRunTime.Storage ObjectStorageMetadata=DeletedStorageInstance.ObjectStorage.StorageMetaData as RDBMSPersistenceRunTime.Storage;
				...
				if(Association.LinkClass==null)
				{
					...
					{
                        RDBMSMetaDataRepository.AssociationEnd mAssociationEnd = ((RDBMSPersistenceRunTime.Storage)DeletedStorageInstance.ObjectStorage.StorageMetaData).GetEquivalentMetaObject(theAssociationEnd) as RDBMSMetaDataRepository.AssociationEnd;
						...
						{
							ObjectStorage objectStorage=(ObjectStorage)ObjectStorage.OpenStorage(OutStorageCell.StorageName,OutStorageCell.StorageLocation,"OOAdvantech.MSSQLPersistenceRunTime.StorageProvider");//Error Prone το πρόγραμμα κάνει την παραδοχή ότι η άλλη στοραγε είναι mssql
