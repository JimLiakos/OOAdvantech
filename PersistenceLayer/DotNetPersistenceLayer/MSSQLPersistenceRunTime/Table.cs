    ...
    public class Table : MetaDataRepository.Namespace
    {
        ...
        protected string GetColumnsDefaultValuesSetScript(string tableName, System.Collections.Generic.List<ColumnDefaultValueData> columnsDefaultValues)
        {
            ...
            foreach (ColumnDefaultValueData columnDefaultValueData in columnsDefaultValues)
            {
                ...
                defaultValuesSetScript += columnDefaultValueData.ColumnName + " = " + TypeDictionary.ConvertToSQLString(columnDefaultValueData.DefaultValue);
            }
        ...
        public void UpdateTableKeys()
        {
            ...
            try
            {
                ...
                foreach (Key key in _Keys)
                {
                    if (key.NewKey)
                    {
                        string addKeyScript =dataBase.RDBMSSchema.GetAddKeyScript(key);//.GetAddKeyScript();
        ...
        public void Update()
        {
            ...
            {
                throw new System.Exception("It can't save the table '" + Name + "' changes.");
            }
            try
            {
                foreach (Key key in _Keys)
