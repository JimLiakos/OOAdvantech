...
namespace OOAdvantech.Linq.Translators
{
    ...
    public static class LinqRuntimeTypeBuilder
    {
        ...
        public static System.Reflection.MethodInfo GetMethod(string methodName, ref System.Linq.Expressions.Expression[] arguments)
        {
            System.Reflection.MethodInfo methodInfo = GetQuarableMethod(methodName, ref  arguments);
            if (methodInfo == null)
                methodInfo = GetEnumerableMethod(methodName, ref  arguments);
            if (methodInfo == null)
                methodInfo = GetObjectsValueCollecorMethod(methodName, ref  arguments);
            return methodInfo;
        }

        public static System.Reflection.MethodInfo GetObjectsValueCollecorMethod(string methodName, ref System.Linq.Expressions.Expression[] arguments)
        {
            int i = 0;
            ...
            var methods = (from method in typeof(ObjectValuesCollector).GetMetaData().GetMethods()
            ...
            foreach (var method in methods)
            {
                ...
                if (method.GetParameters().Length == arguments.Length)
                {
                    ...
                    {
                        if (!isTheSameType(method.GetParameters()[i].ParameterType, arguments[i].Type, typeInstantiation))
                        {
                    ...
                    if (findedMethod != null)
                    {
                        ...
                        foreach (var expression in arguments)
                        {
                            ...
                            {
                                LambdaExpression lambda = ((expression as UnaryExpression).Operand as LambdaExpression);
                                arguments[i] = Expression.Lambda(findedMethod.GetParameters()[i].ParameterType.GetMetaData().GetGenericArguments()[0], lambda.Body, lambda.Parameters);
        ...
        public static System.Reflection.MethodInfo GetQuarableMethod(string methodName, ref System.Linq.Expressions.Expression[] arguments)
        {
            var methods = (from method in typeof(Queryable).GetMetaData().GetMethods()
                           where method.Name == methodName
                           select method).ToArray();
            foreach (var method in methods)
            {
                ...
                if (method.GetParameters().Length == arguments.Length)
                {
                    ...
                    if (findedMethod != null)
                    {
                        ...
                        foreach (var expression in arguments)
                        {
                            ...
                            {
                                LambdaExpression lambda = ((expression as UnaryExpression).Operand as LambdaExpression);
                                arguments[i] = Expression.Quote(Expression.Lambda(findedMethod.GetParameters()[i].ParameterType.GetMetaData().GetGenericArguments()[0], lambda.Body, lambda.Parameters));
        ...
        public static System.Reflection.MethodInfo GetEnumerableMethod(string methodName, ref System.Linq.Expressions.Expression[] arguments)
        {
            int i = 0;
            foreach (var expression in arguments)
            {
            ...
            var methods = (from method in typeof(Enumerable).GetMetaData().GetMethods()
                           where method.Name == methodName
                           select method).ToArray();
            foreach (var method in methods)
            {
        ...
        private static bool isTheSameType(Type type, Type type_2, Dictionary<Type, Type> typeInstantiation)
        {
            if (type_2 == null || type == null)
                return false;
            if (type.Name == type_2.Name && type.GetMetaData().IsGenericType == type_2.GetMetaData().IsGenericType && type.GetMetaData().GetGenericArguments().Length == type_2.GetMetaData().GetGenericArguments().Length)
            {
                bool isEqual = true;
                for (int i = 0; i < type.GetMetaData().GetGenericArguments().Length; i++)
                {
                    if (!type.GetMetaData().GetGenericArguments()[i].IsGenericParameter)
                    {
                        if (!isTheSameType(type.GetMetaData().GetGenericArguments()[i], type_2.GetMetaData().GetGenericArguments()[i], typeInstantiation))
                            return false;
                    }
                    else
                    {
                        typeInstantiation[type.GetMetaData().GetGenericArguments()[i]] = type_2.GetMetaData().GetGenericArguments()[i];
                    }
                }
                if (isEqual)
                    return true;
            ...
            {
                if (isTheSameType(type, type_2.GetMetaData().BaseType, typeInstantiation))
                    return true;
                else
                {
                    foreach (var _interface in type_2.GetMetaData().GetInterfaces())
                    {
        ...
        public static Type GetDynamicType(Dictionary<string, Type> fields)
        {
            ...
            try
            {
                ...
                BuiltTypes[className] = GetDynamicType(className, fields);
                return BuiltTypes[className];

                TypeBuilder typeBuilder = moduleBuilder.DefineType(className, System.Reflection.TypeAttributes.Public | System.Reflection.TypeAttributes.Class | System.Reflection.TypeAttributes.Serializable);

                foreach (var field in fields)
                    typeBuilder.DefineField(field.Key, field.Value, System.Reflection.FieldAttributes.Public);
        ...
        public static Type GetDynamicType(string className, Dictionary<string, Type> fields)
        {
            ...
            try
            {
                ...
                while (BuiltTypes.ContainsKey(className))
                {
                    Type buildedType = BuiltTypes[className];

                    if (buildedType.GetConstructor(fields.Values.ToArray()) != null)
                    {
                        bool propertiesMismatch = false;
                        foreach (var field in fields)
                        {
                            bool exist = false;
                            foreach (System.Reflection.PropertyInfo propertyInfo in buildedType.GetProperties())
                            {
                                if (propertyInfo.Name == field.Key && propertyInfo.PropertyType == field.Value)
                                {
                                    exist = true;
                                    break;
                                }
                        ...
                        className = orgClassName + "_" + (count++).ToString();
                ...
                ConstructorBuilder myConstructorBuilder =
                   typeBuilder.DefineConstructor(System.Reflection.MethodAttributes.Public,
                                      System.Reflection.CallingConventions.Standard, constructorArgs);

                int i = 0;
                foreach (var field in fields)
                {

                    myConstructorBuilder.DefineParameter(i + 1, System.Reflection.ParameterAttributes.HasDefault, field.Key);
                    i++;
                }
        ...
        {
            const System.Reflection.MethodAttributes getSetAttr = System.Reflection.MethodAttributes.Public | System.Reflection.MethodAttributes.HideBySig | System.Reflection.MethodAttributes.SpecialName;

            FieldBuilder field = typeBuilder.DefineField("_" + propertyName, propertyType, System.Reflection.FieldAttributes.Private);
        ...
        public static Type GetDynamicType(IEnumerable<System.Reflection.PropertyInfo> fields)
        {
        ...
        private static string GetTypeKey(IEnumerable<System.Reflection.PropertyInfo> fields)
        {
        ...
        internal static MethodCallExpression GetSelectManyExprsession(Expression source, LambdaExpression selector, LambdaExpression results)
        {
            ...
            System.Reflection.MethodInfo methodInfo = GetMethod("SelectMany", ref arguments);
    ...
    //            {
    //                Expression derivedMemberQueryExpression = (memberExpression.Member.GetCustomAttributes(typeof(MetaDataRepository.DerivedMember), false)[0] as MetaDataRepository.DerivedMember).Expression;
    //                if (OOAdvantech.TypeHelper.GetElementType(derivedMemberQueryExpression.Type) != OOAdvantech.TypeHelper.GetElementType(memberExpression.Type))
    //                    throw new System.Exception(string.Format("Type mismatch between derived member '{0}.{1}' and derived member query expression", memberExpression.Member.DeclaringType.FullName, memberExpression.Member.Name));
    ...
    //        {
    //            MethodCallExpression mergedSelectManyExpression = ExtendSeletcManyExpressionWithSelectManyDerivedMember(orgSelectManyExpression, derivedMemberQueryExpresion);
    //            return GetSelectExpressionWithOrgMethodCallExpressionType(orgSelectManyExpression, mergedSelectManyExpression);
    //        }
    //        else if ((derivedMemberQueryExpresion as MethodCallExpression).Method.Name == "Select")
    //        {
    //            MethodCallExpression mergedSelectExpression = ExtendSelectManyExpressionWithSelectDerivedMember(orgSelectManyExpression, derivedMemberQueryExpresion);
    //            return GetSelectExpressionWithOrgMethodCallExpressionType(orgSelectManyExpression, mergedSelectExpression);
    //        }
    //        else
    ...
    //    /// <summary>
    //    /// Creates a select expression with type with the same properties like the type of originan methodCall resultType
    //    /// </summary>
    //    /// <param name="orgMethodCallExpression">
    //    /// Defines the originan method call expression 
    //    /// </param>
    //    /// <param name="extendedMethodCallExpression">
    //    /// Defines the extended expression with the derivedMemberQueryExpression
    //    /// </param>
    //    /// <returns>
    //    /// Return the select expression
    //    /// </returns>
    //    /// <MetaDataID>{194bfb53-a62e-4142-ac53-39a1df39b76c}</MetaDataID>
    //    private MethodCallExpression GetSelectExpressionWithOrgMethodCallExpressionType(MethodCallExpression orgMethodCallExpression, MethodCallExpression extendedMethodCallExpression)
    //    {
    //        Type methodCallResultType = OOAdvantech.TypeHelper.GetElementType(extendedMethodCallExpression.Type);
    ...
    //        Type selectionResultType = OOAdvantech.TypeHelper.GetElementType(orgMethodCallExpression.Type);

    //        Expression orgResultSelector = orgMethodCallExpression.Arguments[orgMethodCallExpression.Arguments.Count - 1];

    //        Expression[] selectionArguments = new Expression[2];
    //        selectionArguments[0] = extendedMethodCallExpression;
    //        if (GetLambdaExpression(orgResultSelector).Body is NewExpression)
    //        {

    //            Expression[] selectionResultTypeArguments = (((orgMethodCallExpression.Arguments[2] as UnaryExpression).Operand as LambdaExpression).Body as NewExpression).Arguments.ToArray();

    //            ParameterExpression selectionSourceParameter = Expression.Parameter(methodCallResultType, GetLambdaExpression(orgResultSelector).Parameters[0].Name + "_1");

    //            for (int i = 0; i != selectionResultTypeArguments.Length; i++)
    //            {
    //                path.Clear();
    //                Expression argExpression = selectionResultTypeArguments[i];
    //                path = GetPath(argExpression);
    //                path.Insert(0, GetParameter(argExpression).Name);
    ...
    //            Expression selectorResultsBody = Expression.New(selectionResultType.GetConstructors()[0], selectionResultTypeArguments);


    //            selectionArguments[1] = Expression.Quote(Expression.Lambda(selectorResultsBody, selectionSourceParameter));
    //        }
    //        else
    //        {

    //            Expression argExpression = GetLambdaExpression(orgResultSelector).Body;
    //            path = GetPath(argExpression);
    //            path.Insert(0, GetParameter(argExpression).Name);
    ...
    //            else
    //                selectionArguments[1] = Expression.Quote(Expression.Lambda(argExpression, Expression.Parameter(methodCallResultType, GetLambdaExpression(orgResultSelector).Parameters[0].Name + "_1")));

    //        }

    //        MethodInfo methodInfo = LinqRuntimeTypeBuilder.GetMethod("Select", ref selectionArguments);
    ...
    //    /// <summary>
    //    /// Extend the select expression with derived member expression
    //    /// </summary>
    //    /// <param name="orgSelectExpression">
    //    /// Defines the select expression of main query
    //    /// </param>
    //    /// <param name="derivedMemberQueryExpresion">
    //    /// Defines the query expresion of derived member 
    //    /// </param>
    //    /// <returns>
    //    /// Returns the select expression with same type like the original select expression
    //    /// </returns>
    //    /// <MetaDataID>{54242dda-b540-4653-8c94-8ee3d09ba35a}</MetaDataID>
    //    private MethodCallExpression ExtendSelectManyExpressionWithSelectDerivedMember(MethodCallExpression orgSelectExpression, System.Linq.Expressions.MethodCallExpression derivedMemberQueryExpresion)
    //    {
    //        Expression orgSource = orgSelectExpression.Arguments[0];
    //        Expression orgCollectionSelector = orgSelectExpression.Arguments[1];
    ...
    //        ParameterExpression orgDerivedMemberParam = ((orgSelectExpression.Arguments[2] as UnaryExpression).Operand as LambdaExpression).Parameters[1] as ParameterExpression;
    //        MemberExpression orgDerivedMemberAccessExpression = GetDerividedMemberAccess(orgSelectExpression.Arguments[1]);


    //        List<string> path = new List<string>();
    //        Expression source = null;
    //        Expression selector = null;
    //        Expression[] selectArguments = new Expression[2];

    //        if (derivedMemberQueryExpresion.Arguments[0] is MethodCallExpression)
    //            source = ExtendMainQueryWithDerivedMemberQueryExpression(derivedMemberQueryExpresion.Arguments[0] as MethodCallExpression, orgSource, orgDerivedMemberAccessExpression);
    //        else
    //            source = ExtendMainQueryWithDerivedMemberQueryExpression(derivedMemberQueryExpresion, orgSource, orgDerivedMemberAccessExpression);


    //        #region Builds mergedExpression result selector

    //        //Expression derivedMemberSelector = derivedMemberQueryExpresion.Arguments[1];
    //        //derivedMemberSelector = ReplaceSourceType(derivedMemberSelector, OOAdvantech.TypeHelper.GetElementType(derivedMemberQueryExpresion.Arguments[0].Type), OOAdvantech.TypeHelper.GetElementType(source.Type));
    //        //ParameterExpression sourceParam = ((derivedMemberSelector as UnaryExpression).Operand as LambdaExpression).Parameters[0];


    //        Expression derivedMemberSelector = derivedMemberQueryExpresion.Arguments[1];

    //        ParameterExpression sourceParam =Expression.Parameter(OOAdvantech.TypeHelper.GetElementType(source.Type), GetLambdaExpression(derivedMemberSelector).Parameters[0].Name);
    //        derivedMemberSelector = Expression.Quote(Expression.Lambda(ReplaceParameter(GetLambdaExpression(derivedMemberSelector).Body, GetLambdaExpression(derivedMemberSelector).Parameters[0], sourceParam), sourceParam));




    //        //derivedMemberSelector = ReplaceParameter(derivedMemberSelector, sourceParam);
    //        GetDerivedMemberSourcePath(sourceParam.Type, orgSourceCollectionParam, path);

    //        // mergedExpressionResultSelector_SourcePropertyInitiator initiate property for orgExpresion source sequence element type 
    //        Expression resultSelector_SourcePropertyInitiator = sourceParam;
    //        foreach (string memberName in path)
    //            resultSelector_SourcePropertyInitiator = Expression.PropertyOrField(resultSelector_SourcePropertyInitiator, memberName);

    //        // mergedResultSelector_DerivedMemberInitiator initiate property for derived member sequence element type 
    //        Expression resultSelector_DerivedMemberInitiator = GetLambdaExpression(derivedMemberSelector).Body;

    //        Type selectResultType = OOAdvantech.TypeHelper.GetElementType(orgSelectExpression.Type);
    //        var selectResultTypeMembers = new Dictionary<string, Type>() { { orgSourceCollectionParam.Name, resultSelector_SourcePropertyInitiator.Type},
    //                                                                                {orgDerivedMemberParam.Name,resultSelector_DerivedMemberInitiator.Type}};

    //        selectResultType = LinqRuntimeTypeBuilder.GetDynamicType(selectResultType.Name + "Source", selectResultTypeMembers);
    //        //mergedExpressionResultSelector_SourcePropertyInitiator

    //        var selectorBody = Expression.New(selectResultType.GetConstructors()[0], resultSelector_SourcePropertyInitiator, resultSelector_DerivedMemberInitiator);
    //        selector = Expression.Quote(Expression.Lambda(selectorBody, sourceParam));
    //        #endregion

    //        selectArguments[0] = source;
    //        selectArguments[1] = selector;
    //        System.Reflection.MethodInfo methodInfo = LinqRuntimeTypeBuilder.GetMethod("Select", ref selectArguments);
    //        MethodCallExpression mergedSelectManyExpression = Expression.Call(methodInfo, selectArguments);
    //        return mergedSelectManyExpression;
    //    }
    //    /// <summary>
    //    /// Parse the expresion in case where find the original parameter then replace it with the new
    //    /// </summary>
    //    /// <param name="expression">
    //    /// Defines the expression which method parse
    //    /// </param>
    //    /// <param name="orgParameter">
    //    /// Defines the original parameter 
    //    /// </param>
    //    /// <param name="newParameter">
    //    /// Defines the new parameter
    //    /// </param>
    //    /// <returns>
    //    /// Returns the new expression in case where method found the orgPrameter and replace  with new parameter
    //    /// in other case  returns the original expression.
    //    /// </returns>
    //    private Expression ReplaceParameter(Expression expression, ParameterExpression orgParameter, ParameterExpression newParameter)
    //    {
    //        if (expression is UnaryExpression || expression is LambdaExpression)
    //        {
    //            ParameterExpression[] parameters= GetLambdaExpression(expression).Parameters.ToArray();
    //            bool replaceParameters=false;
    //            for (int i = 0; i != parameters.Length; i++)
    //            {
    //                if (parameters[i] == orgParameter)
    //                {
    ...
    //            {
    //                Expression body = ReplaceParameter( GetLambdaExpression(expression).Body,orgParameter,newParameter);
    //                if (expression is UnaryExpression)
    ...
    //        {
    //            if (GetParameter(expression) == orgParameter)
    //            {
    //                List<string> path = GetPath(expression);
    //                Expression newExpression = newParameter;
    //                foreach (var memberName in path)
    //                    newExpression = Expression.PropertyOrField(newExpression, memberName);
    ...
    //        {
    //            if (expression as ParameterExpression == orgParameter)
    //                return newParameter;
    //            else
    //                return expression;
    //        }
    ...
    //            foreach (var arg in (expression as NewExpression).Arguments)
    //                Args.Add(ReplaceParameter(arg, orgParameter, newParameter));
    //            Expression.New((expression as NewExpression).Constructor, Args);
    //        }
    //        if (expression is MemberInitExpression)
    //        {
    //            List<MemberBinding> memberBindings = new List<MemberBinding>();

    //            foreach (MemberBinding memberBinding in (expression as MemberInitExpression).Bindings)
    //            {
    //                if (memberBinding is MemberAssignment)
    //                {
    //                    Expression memberBindingExpression = ReplaceParameter((memberBinding as MemberAssignment).Expression, orgParameter, newParameter);
    //                    memberBindings.Add(Expression.Bind(memberBinding.Member, memberBindingExpression));
    //                }
    //                else
    //                    memberBindings.Add(memberBinding);
    //            }
    //            return Expression.MemberInit((expression as MemberInitExpression).NewExpression, memberBindings);
    //        }
    //        if (expression is BinaryExpression)
    //            return Expression.MakeBinary((expression as BinaryExpression).NodeType, ReplaceParameter((expression as BinaryExpression).Left, orgParameter, newParameter), ReplaceParameter((expression as BinaryExpression).Right, orgParameter, newParameter), (expression as BinaryExpression).IsLiftedToNull, (expression as BinaryExpression).Method);
    //        return expression;

    //    }
    //    /// <summary>
    //    /// This method rebuilds expression with new parameters
    //    /// </summary>
    //    /// <param name="expression">
    //    /// Defines the expression which method rebuilds
    //    /// </param>
    //    /// <param name="parameterExpressions">
    //    /// Defines the new parameters which method uses to rebuilds the expression
    //    /// </param>
    //    /// <returns>
    //    /// In case where method can rebuilds exression with new parameters returns the new expression
    //    /// in other case returns null
    //    /// </returns>
    //    private Expression ReplaceParameter(Expression expression, ParameterExpression[] parameterExpressions)
    //    {
    //        if (expression is MemberExpression)
    //        {
    //            foreach (var parameterExpression in parameterExpressions)
    //            {
    //                if (GetParameter(expression) == parameterExpression)
    //                    return expression;
    //                else
    //                {
    //                    Expression newExpression = parameterExpression;
    //                    List<string> path = GetPath(expression);
    //                    if (parameterExpression.Type.GetProperty(path[0]) != null &&
    //                        parameterExpression.Type.GetProperty(path[0]).PropertyType == GetParameter(expression).Type.GetProperty(path[0]).PropertyType)
    //                    {
    //                        foreach (var memberName in path)
    //                            newExpression = Expression.PropertyOrField(newExpression, memberName);
    //                        return newExpression;
    ...
    //                    else
    //                    {
    //                        path.Insert(0, GetParameter(expression).Name);
    //                        if (parameterExpression.Type.GetProperty(GetParameter(expression).Name) != null &&
    //                            parameterExpression.Type.GetProperty(GetParameter(expression).Name).PropertyType == GetParameter(expression).Type)
    //                        {
    //                            foreach (var memberName in path)
    //                                newExpression = Expression.PropertyOrField(newExpression, memberName);
    //                            return newExpression;
    ...
    //            foreach (var parameterExpression in parameterExpressions)
    //            {
    //                if (parameterExpression.Type == expression.Type)
    //                    return parameterExpression;
    //                if (parameterExpression.Type.GetProperty((expression as ParameterExpression).Name) != null &&
    //                    parameterExpression.Type.GetProperty((expression as ParameterExpression).Name).PropertyType == expression.Type)
    //                {
    //                    List<string> path = GetPath(expression);
    //                    path.Insert(0, (expression as ParameterExpression).Name);
    //                    Expression newExpresion = parameterExpression;
    //                    foreach (string memberName in path)
    //                        newExpresion = Expression.PropertyOrField(newExpresion, memberName);
    //                    return newExpresion;
    //                }

    //            }
    //            return null;
    //            //    && sourceParam != (expression as ParameterExpression)
    //            //    && sourceParam.Type != (expression as ParameterExpression).Type)
    //            //{
    //            //    return sourceParam;
    //            //}
    //        }
    ...
    //            List<Expression> Args = new List<Expression>();
    //            foreach (var arg in (expression as NewExpression).Arguments)
    //                Args.Add(ReplaceParameter(arg, parameterExpressions));
    //            Expression.New((expression as NewExpression).Constructor, Args);
    //        }
    //        if (expression is MemberInitExpression)
    //        {
    ...
    //                if (memberBinding is MemberAssignment)
    //                {
    //                    Expression memberBindingExpression = ReplaceParameter((memberBinding as MemberAssignment).Expression, parameterExpressions);
    //                    memberBindings.Add(Expression.Bind(memberBinding.Member, memberBindingExpression));
    //                }
    //                else
    //                    memberBindings.Add(memberBinding);
    ...
    //        //    return Expression.MakeBinary((expression as BinaryExpression).NodeType, ReplaceParameter((expression as BinaryExpression).Left, sourceParam), ReplaceParameter((expression as BinaryExpression).Right, sourceParam), (expression as BinaryExpression).IsLiftedToNull, (expression as BinaryExpression).Method);
    //        //}
    ...
    //    /// Extend the select expression with derived member expression
    //    /// </summary>
    //    /// <param name="orgSelectManyExpression">
    //    /// Defines the select many expression of main query
    //    /// </param>
    //    /// <param name="derivedMemberQueryExpresion">
    //    /// Defines the query expresion of derived member 
    //    /// </param>
    //    /// <returns>
    //    /// Returns the select expression with same type like the original select many expression
    //    /// </returns>
    //    /// <MetaDataID>{6f84c033-3ba4-4241-882a-371b10ae023f}</MetaDataID>
    //    private MethodCallExpression ExtendSeletcManyExpressionWithSelectManyDerivedMember(MethodCallExpression orgSelectManyExpression, System.Linq.Expressions.MethodCallExpression derivedMemberQueryExpresion)
    //    {
    ...
    //        ParameterExpression orgSourceCollectionParam = ((orgCollectionSelector as UnaryExpression).Operand as LambdaExpression).Parameters[0] as ParameterExpression;
    //        MemberExpression orgDerivedMemberCollectionSelector = GetDerividedMemberAccess(orgSelectManyExpression.Arguments[1]);

    //        Expression source = null;
    //        Expression collectionSelector = null;
    //        Expression collectionSelectorBody = null;
    ...
    //        if (derivedMemberQueryExpresion.Arguments[0] is MethodCallExpression)
    //        {
    //            //defines the source collection expression for the new select  expression 
    //            source = ExtendMainQueryWithDerivedMemberQueryExpression(derivedMemberQueryExpresion.Arguments[0] as MethodCallExpression, orgSource, orgDerivedMemberCollectionSelector);

    //            //builds collection selector expression for the necessary data to build the derivemamber Type
    //            Expression derivedMemberQueryExpressionCollectioSelector = derivedMemberQueryExpresion.Arguments[1];
    //            //extract body and parameter from old expression
    //            ParameterExpression oldSourceParam=GetLambdaExpression(derivedMemberQueryExpressionCollectioSelector).Parameters[0];
    //            collectionSelectorBody = GetLambdaExpression(derivedMemberQueryExpressionCollectioSelector).Body;

    //            sourceParam = Expression.Parameter(OOAdvantech.TypeHelper.GetElementType(source.Type), oldSourceParam.Name);
    //            collectionSelectorBody = ReplaceParameter(collectionSelectorBody, oldSourceParam, sourceParam);

    //            collectionSelector = Expression.Quote(Expression.Lambda(collectionSelectorBody, sourceParam));
    //        }
    //        else
    //        {
    ...
    //            collectionSelectorBody = orgDerivedMemberCollectionSelector.Expression;
    //            sourceParam = GetParameter(collectionSelectorBody);

    //            Expression derivedMemberQuerySelector = GetLambdaExpression(derivedMemberQueryExpresion.Arguments[1]).Body;
    //            path = GetPath(derivedMemberQuerySelector);
    //            foreach (var memberName in path)
    //                collectionSelectorBody = Expression.PropertyOrField(collectionSelectorBody, memberName);
    ...
    //        collectionSelectorBody = ((collectionSelector as UnaryExpression).Operand as LambdaExpression).Body;
    //        // Defines source sequence item parameter which use the collection selector to retrivie collection 
    //        sourceParam = ((collectionSelector as UnaryExpression).Operand as LambdaExpression).Parameters[0];
    //        //Defines selector collection item derivedMemberQueryExpresioncollectionSelectorParam used from result selector in conjunction with  mergedExpresionCollectionSelectorParam to produce SelectMany result 
    //        ParameterExpression derivedMemberQueryExpresionCollectionSelectorParam = ((derivedMemberQueryExpresion.Arguments[2] as UnaryExpression).Operand as LambdaExpression).Parameters[1];
    ...
    //        ParameterExpression collectionSelectorParm = derivedMemberQueryExpresionCollectionSelectorParam;

    //        // mergedResultSelector_DerivedMemberInitiator initiate property for derived member sequence element type 
    //        Expression resultSelector_DerivedMemberInitiator = GetLambdaExpression(derivedMemberQueryExpresion.Arguments[2], new ParameterExpression[] { sourceParam, collectionSelectorParm }).Body;

    //        var selectResultTypeMembers = new Dictionary<string, Type>() { { ((orgSelectManyExpression.Arguments[2] as UnaryExpression).Operand as LambdaExpression).Parameters[0].Name, resultSelector_SourcePropertyInitiator.Type},
    //                                                    {((orgSelectManyExpression.Arguments[2] as UnaryExpression).Operand as LambdaExpression).Parameters[1].Name,resultSelector_DerivedMemberInitiator.Type}};

    //        Type selectResultType = LinqRuntimeTypeBuilder.GetDynamicType(OOAdvantech.TypeHelper.GetElementType(orgSelectManyExpression.Type).Name + "Source", selectResultTypeMembers);

    //        var resultSelectorBody = Expression.New(selectResultType.GetConstructors()[0], resultSelector_SourcePropertyInitiator, resultSelector_DerivedMemberInitiator);

    //        resultSelector = Expression.Quote(Expression.Lambda(resultSelectorBody, sourceParam, collectionSelectorParm));

    //        #endregion



    //        Expression[] selectManyArguments = new Expression[3] { source, collectionSelector, resultSelector };
    //        System.Reflection.MethodInfo methodInfo = LinqRuntimeTypeBuilder.GetMethod("SelectMany", ref selectManyArguments);
    //        MethodCallExpression mergedSelectManyExpression = Expression.Call(methodInfo, selectManyArguments);
    //        return mergedSelectManyExpression;
    //    }
    ...
    //    /// </returns>
    //    /// <MetaDataID>{54242dda-b540-4653-8c94-8ee3d09ba35a}</MetaDataID>
    //    private MethodCallExpression ExtendSelectExpressionWithSelectDerivedMember(MethodCallExpression orgSelectExpression, System.Linq.Expressions.MethodCallExpression derivedMemberQueryExpresion)
    //    {
    //        Expression orgSource = orgSelectExpression.Arguments[0];
    //        ParameterExpression orgSourceParam = GetParameter(orgSource);// ((orgSource as UnaryExpression).Operand as LambdaExpression).Parameters[0] as ParameterExpression;
    //        Expression orgResultSelector = orgSelectExpression.Arguments[1];
    //        ParameterExpression orgDerivedMemberParam = (orgResultSelector as LambdaExpression).Parameters[0] as ParameterExpression;

    //        MemberExpression orgDerivedMemberCollectionSelector = GetDerividedMemberAccess(orgSelectExpression.Arguments[0]);
    ...
    //            source = ExtendMainQueryWithDerivedMemberQueryExpression(derivedMemberQueryExpresion.Arguments[0] as MethodCallExpression, orgSource, orgDerivedMemberCollectionSelector);
    //        else
    //            source = ExtendMainQueryWithDerivedMemberQueryExpression(derivedMemberQueryExpresion, orgSource, orgDerivedMemberCollectionSelector);


    //        #region Builds mergedExpression result selector

    //        Expression derivedMemberQueryExpressionResultSelector = derivedMemberQueryExpresion.Arguments[1];
    //        ParameterExpression sourceParam = Expression.Parameter(OOAdvantech.TypeHelper.GetElementType(source.Type), GetLambdaExpression(derivedMemberQueryExpressionResultSelector).Parameters[0].Name);
    //        derivedMemberQueryExpressionResultSelector = Expression.Quote(Expression.Lambda(ReplaceParameter(GetLambdaExpression(derivedMemberQueryExpressionResultSelector).Body, GetLambdaExpression(derivedMemberQueryExpressionResultSelector).Parameters[0], sourceParam), sourceParam));

    //        //Expression derivedMemberQueryExpressionResultSelector = derivedMemberQueryExpresion.Arguments[1];
    //        //derivedMemberQueryExpressionResultSelector = ReplaceSourceType(derivedMemberQueryExpressionResultSelector, OOAdvantech.TypeHelper.GetElementType(derivedMemberQueryExpresion.Arguments[0].Type), OOAdvantech.TypeHelper.GetElementType(source.Type));
    //        //ParameterExpression sourceParam = ((derivedMemberQueryExpressionResultSelector as UnaryExpression).Operand as LambdaExpression).Parameters[0];
    ...
    //        // mergedResultSelector_DerivedMemberInitiator initiate property for derived member sequence element type 
    //        Expression resultSelector_DerivedMemberInitiator = GetLambdaExpression(derivedMemberQueryExpressionResultSelector).Body;


    //        Dictionary<string, Type> selectResultTypeMembers = new Dictionary<string, Type>() { { orgSourceParam.Name, resultSelector_SourcePropertyInitiator.Type},
    //                                           {orgDerivedMemberParam.Name,resultSelector_DerivedMemberInitiator.Type}};

    //        Type selectResultType = LinqRuntimeTypeBuilder.GetDynamicType(TypeHelper.GetElementType(orgSelectExpression.Type).Name + "Source", selectResultTypeMembers);

    //        var selectorBody = Expression.New(selectResultType.GetConstructors()[0], resultSelector_SourcePropertyInitiator, resultSelector_DerivedMemberInitiator);
    //        selector = Expression.Quote(Expression.Lambda(selectorBody, sourceParam));
    //        #endregion

    //        Expression[] selectArguments = new Expression[2] { source, selector };
    //        System.Reflection.MethodInfo methodInfo = LinqRuntimeTypeBuilder.GetMethod("Select", ref selectArguments);
    //        MethodCallExpression mergedSelectManyExpression = Expression.Call(methodInfo, selectArguments);
    //        return mergedSelectManyExpression;
    //    }
    ...
    //    /// </returns>
    //    /// <MetaDataID>{6f84c033-3ba4-4241-882a-371b10ae023f}</MetaDataID>
    //    private MethodCallExpression ExtendSeletcExpressionWithSelectManyDerivedMember(MethodCallExpression orgSelectExpression, System.Linq.Expressions.MethodCallExpression derivedMemberQueryExpresion)
    //    {
    ...
    //        Expression derivedMemberQueryExpressionResultSelector = derivedMemberQueryExpresion.Arguments[2];
    ...
    //        #region Builds mergedExpression result selector
    //        ParameterExpression sourceParam = Expression.Parameter(OOAdvantech.TypeHelper.GetElementType(source.Type), GetLambdaExpression(derivedMemberQueryExpresion.Arguments[1]).Parameters[0].Name + "_1");

    //        // mergedExpressionResultSelector_SourcePropertyInitiator initiate property for orgExpresion source sequence element type 
    //        Expression resultSelector_SourcePropertyInitiator = sourceParam;

    //        // mergedResultSelector_DerivedMemberInitiator initiate property for derived member sequence element type 
    //        Expression resultSelector_DerivedMemberInitiator = GetLambdaExpression(derivedMemberQueryExpressionResultSelector, new ParameterExpression[] { sourceParam }).Body;
    ...
    //        Type selectResultType = LinqRuntimeTypeBuilder.GetDynamicType(OOAdvantech.TypeHelper.GetElementType(orgSelectExpression.Type).Name + "Source", selectResultTypeMembers);

    //        Expression selectorBody = Expression.New(selectResultType.GetConstructors()[0], resultSelector_SourcePropertyInitiator, resultSelector_DerivedMemberInitiator);
    //        selector = Expression.Quote(Expression.Lambda(selectorBody, sourceParam));
    //        #endregion
    ...
    //    /// <summary>
    //    /// Gets the lambda expresion and replace the org parameters with new parameters
    //    /// </summary>
    //    /// <param name="expression">
    //    /// Defines the expression which is Lambda expression or contains the Lambda expression 
    //    /// </param>
    //    /// <param name="parameterExpressions">
    //    /// Defines an array with new parameters
    //    /// </param>
    //    /// <returns></returns>
    //    private LambdaExpression GetLambdaExpression(Expression expression, ParameterExpression[] parameterExpressions)
    //    {
    //        if (expression is LambdaExpression)
    //        {
    //            return Expression.Lambda(ReplaceParameter((expression as LambdaExpression).Body, parameterExpressions), parameterExpressions);
    //        }
    //        if (expression is UnaryExpression)
    //            return GetLambdaExpression((expression as UnaryExpression).Operand, parameterExpressions);
    //        return null;
    ...
    //        System.Linq.Expressions.MethodCallExpression derivedMemberQueryExpresion = (GetDerividedMemberAccess(orgSelectExpression.Arguments[0]).Member.GetCustomAttributes(typeof(OOAdvantech.MetaDataRepository.DerivedMember), false)[0] as OOAdvantech.MetaDataRepository.DerivedMember).Expression as System.Linq.Expressions.MethodCallExpression;

    //        if ((derivedMemberQueryExpresion as MethodCallExpression).Method.Name == "SelectMany")
    //        {
    //            MethodCallExpression mergedSelectManyExpression = ExtendSeletcExpressionWithSelectManyDerivedMember(orgSelectExpression, derivedMemberQueryExpresion);
    //            return GetSelectExpressionWithOrgMethodCallExpressionType(orgSelectExpression, mergedSelectManyExpression);
    //        }
    //        else if ((derivedMemberQueryExpresion as MethodCallExpression).Method.Name == "Select")
    //        {
    //            MethodCallExpression mergedSelectExpression = ExtendSelectExpressionWithSelectDerivedMember(orgSelectExpression, derivedMemberQueryExpresion);
    //            return GetSelectExpressionWithOrgMethodCallExpressionType(orgSelectExpression, mergedSelectExpression);
    //        }
    //        else
    ...
    //    private Expression ExtendMainQueryWithDerivedMemberQueryExpression(MethodCallExpression derivedMemberExpresion, Expression orgSource, MemberExpression orgDerivedMemberCollectionSelector)
    //    {

    //        if ((derivedMemberExpresion as MethodCallExpression).Arguments[0] is MethodCallExpression)
    //        {
    //            System.Linq.Expressions.Expression extendedExpresionSource = ExtendMainQueryWithDerivedMemberQueryExpression(derivedMemberExpresion.Arguments[0] as MethodCallExpression, orgSource, orgDerivedMemberCollectionSelector);
    //            System.Linq.Expressions.Expression extendedExpresion = ReplaceMethodCallSource((derivedMemberExpresion as MethodCallExpression), extendedExpresionSource);
    ...
    //        {
    //            //Constant collection
    //            Expression derivedExpressionCollectionSelector = GetLambdaExpression(derivedMemberExpresion.Arguments[1]).Body;

    //            //The element type of Derived member query source "OOAdvantech.Linq.DerivedMembersExpresionBuilder<T>.ObjectCollection"
    //            //must be the same with the main query, derived member collection selector.

    //            if (GetMemberOwner(derivedExpressionCollectionSelector).Type != GetMemberOwner(orgDerivedMemberCollectionSelector).Type)
    //                throw new System.Exception(string.Format("Derived member owner and derived member query expression source, type mismatch  '{0}'", orgDerivedMemberCollectionSelector.Member.DeclaringType.FullName + "." + orgDerivedMemberCollectionSelector.Member.Name));


    //            if ((orgSource is MemberExpression) && (orgSource as MemberExpression).Expression.Type == (derivedExpressionCollectionSelector as MemberExpression).Expression.Type)
    //            {

    //                Expression orgDerivedMemberCollectionSelectorOwner = orgDerivedMemberCollectionSelector.Expression;

    //                // original source is object and and not collection of objects

    //                List<string> path = GetPath(derivedExpressionCollectionSelector);


    //                //merge orgDerivedMemberSelector with derived member query expression collectionSelector 
    //                var mergeSource = orgDerivedMemberCollectionSelectorOwner;
    //                foreach (var memberName in path)
    //                    mergeSource = Expression.PropertyOrField(mergeSource, memberName);


    //                Type resultType = null;
    //                Dictionary<string, Type> dynamicTypeParameters = new Dictionary<string, Type>();

    //                Expression derivedExpressionResultSelctor = derivedMemberExpresion.Arguments[2];
    //                foreach (var parameter in GetLambdaExpression(derivedExpressionResultSelctor).Parameters)
    //                    dynamicTypeParameters.Add(parameter.Name, parameter.Type);


    //                var mergeSourceParam = GetLambdaExpression(derivedExpressionResultSelctor).Parameters[1];
    //                resultType = LinqRuntimeTypeBuilder.GetDynamicType(GetLambdaExpression(derivedExpressionResultSelctor).Body.Type.Name + "DeriveMember", dynamicTypeParameters);

    //                Expression mergeResultsBody = Expression.New(resultType.GetConstructors()[0], orgDerivedMemberCollectionSelectorOwner, mergeSourceParam);
    //                var mergeResults = Expression.Quote(Expression.Lambda(mergeResultsBody, mergeSourceParam));


    //                Expression[] selectionArguments = new Expression[2] { mergeSource, mergeResults };
    //                MethodInfo methodInfo = LinqRuntimeTypeBuilder.GetMethod("Select", ref selectionArguments);
    //                return Expression.Call(methodInfo, selectionArguments);
    ...
    //                bool hasTheSameSourceParameterName = GetParameter(orgDerivedMemberCollectionSelectorOwner).Name == GetParameter(derivedExpressionCollectionSelector).Name;


    //                Type resultType = null;
    //                Dictionary<string, Type> dynamicTypeParameters = new Dictionary<string, Type>();
    //                if (!hasTheSameSourceParameterName)
    //                    dynamicTypeParameters.Add(GetParameter(orgDerivedMemberCollectionSelectorOwner).Name, GetParameter(orgDerivedMemberCollectionSelectorOwner).Type);
    //                foreach (var parameter in ((derivedMemberExpresion.Arguments[2] as UnaryExpression).Operand as LambdaExpression).Parameters)
    //                    dynamicTypeParameters.Add(parameter.Name, parameter.Type);
    //                var mergeSourceParam = Expression.Parameter(GetParameter(orgDerivedMemberCollectionSelectorOwner).Type, GetParameter(orgDerivedMemberCollectionSelectorOwner).Name);

    //                #region Builds selector Body

    //                Expression mergeSelectorBody = mergeSourceParam;
    //                List<string> path = GetPath(orgDerivedMemberCollectionSelectorOwner);
    //                foreach (var memberName in path)
    //                    mergeSelectorBody = Expression.PropertyOrField(mergeSelectorBody, memberName);
    //                path = GetPath(derivedExpressionCollectionSelector);
    //                foreach (var memberName in path)
    //                    mergeSelectorBody = Expression.PropertyOrField(mergeSelectorBody, memberName);
    ...
    //                Expression derivedMemberSourceParam = mergeSourceParam;
    //                path = GetPath(orgDerivedMemberCollectionSelectorOwner);
    //                foreach (string memberName in path)
    //                    derivedMemberSourceParam = Expression.PropertyOrField(derivedMemberSourceParam, memberName);
    ...
    //                    extendedExpresion = LinqRuntimeTypeBuilder.GetSelectManyExprsession(orgSource, Expression.Lambda(mergeSelectorBody, mergeSourceParam), mergeResults);
    //                }
    //                else
    //                {
    ...
    //    private Expression GetMemberOwner(Expression typeMemberAccessExxpresion)
    //    {
    //        if (typeMemberAccessExxpresion is MemberExpression)
    //            return (typeMemberAccessExxpresion as MemberExpression).Expression;
    //        else
    //            return null;

    //    }

    //    private static List<string> GetPath(Expression memberExpression)
    //    {
    //        List<string> path = new List<string>();
    //        while (memberExpression is MemberExpression)
    //        {
    //            path.Insert(0, (memberExpression as MemberExpression).Member.Name);
    //            memberExpression = (memberExpression as MemberExpression).Expression;
    //        }
    //        return path;
    //    }
    ...
    //    //    if (expression is LambdaExpression)
    //    //    {
    //    //        Dictionary<ParameterExpression, ParameterExpression> replacedParametrers = new Dictionary<ParameterExpression, ParameterExpression>();
    //    //        var parameters = (expression as LambdaExpression).Parameters.ToArray();
    //    //        int i = 0;
    //    //        foreach (var parameter in parameters)
    //    //        {
    //    //            if (parameter.Type == oldSourceType)
    //    //            {
    //    //                ParameterExpression newParameter = Expression.Parameter(newSourceType, parameter.Name);
    //    //                parameters[i++] = newParameter;
    //    //                replacedParametrers[parameter] = newParameter;
    //    //            }
    //    //            else
    //    //                parameters[i++] = parameter;
    //    //        }

    //    //        Expression body = (expression as LambdaExpression).Body;
    //    //        foreach (var parameterEntry in replacedParametrers)
    //    //            body = ReplaceParameter(body, parameterEntry.Key,parameterEntry.Value);
    //    //        body = ReplaceSourceType(body, oldSourceType, newSourceType);
    ...
    //    ///<summary>
    //    ///This method replace the source of method call expression
    //    ///</summary>
    //    ///<param name="methodCallExpression">
    //    ///Defines the method call expression where will be changed the source (first argument) 
    //    ///</param>
    //    ///<param name="newSourceExpression">
    //    ///Defines the new source expression
    //    ///</param>
    //    ///<returns>
    //    ///Returns the expression with the changes
    //    ///</returns>
    //    /// <MetaDataID>{d5ff5100-4d3b-424c-99fb-d111bb22c70c}</MetaDataID>
    //    private Expression ReplaceMethodCallSource(MethodCallExpression methodCallExpression, Expression newSourceExpression)
    //    {

    //        Expression[] arguments = methodCallExpression.Arguments.ToArray();
    //        Expression orgSource = arguments[0];
    //        arguments[0] = newSourceExpression;
    //        if (orgSource.Type != newSourceExpression.Type)
    //        {
    //            for (int i = 1; i < arguments.Length; i++)
    //            {
    //                LambdaExpression lambdaExpression = GetLambdaExpression(arguments[i]);
    //                if (lambdaExpression != null)
    //                {
    //                    Dictionary<ParameterExpression, ParameterExpression> replcacedParameters = new Dictionary<ParameterExpression, ParameterExpression>();
    //                    foreach (ParameterExpression parameter in lambdaExpression.Parameters)
    //                    {
    //                        if (parameter.Type == OOAdvantech.TypeHelper.GetElementType(orgSource.Type))
    //                        {
    //                            ParameterExpression newParameter = Expression.Parameter(OOAdvantech.TypeHelper.GetElementType(newSourceExpression.Type), parameter.Name);
    //                            replcacedParameters[parameter] = newParameter;
    //                        }
    //                    }
    //                    foreach (var parameterEntry in replcacedParameters)
    //                        arguments[i] = ReplaceParameter(arguments[i], parameterEntry.Key, parameterEntry.Value);
    //                }
    ...
    //            methodInfo = LinqRuntimeTypeBuilder.GetMethod(methodCallExpression.Method.Name, ref arguments);
    //        else
    ...
    //    internal static ParameterExpression GetParameter(Expression expression)
    //    {
    ...
    /// <MetaDataID>{7a274ddd-272f-40d4-a9db-2c15c8ae02fd}</MetaDataID>
    ...
    internal abstract class ExpressionVisitor
    {
        ...
        /// <MetaDataID>{a6a7e0bd-5641-4229-a0f3-ef15f8828b81}</MetaDataID>
        protected UnaryExpression UpdateUnary(UnaryExpression u, Expression operand, Type resultType, System.Reflection.MethodInfo method)
        {
            if (u.Operand != operand || u.Type != resultType || u.Method != method)
            {
                return Expression.MakeUnary(u.NodeType, operand, resultType, method);
        ...
        /// <MetaDataID>{c17403fc-06cb-4982-8a06-331b6f6fcb94}</MetaDataID>
        protected BinaryExpression UpdateBinary(BinaryExpression b, Expression left, Expression right, Expression conversion, bool isLiftedToNull, System.Reflection.MethodInfo method)
        {
            if (left != b.Left || right != b.Right || conversion != b.Conversion || method != b.Method || isLiftedToNull != b.IsLiftedToNull)
            {
                if (b.NodeType == ExpressionType.Coalesce && b.Conversion != null)
        ...
        /// <MetaDataID>{95dbf694-9313-4206-8168-490ea155d0e7}</MetaDataID>
        protected MemberExpression UpdateMemberAccess(MemberExpression m, Expression expression, System.Reflection.MemberInfo member)
        {
            if (expression != m.Expression || member != m.Member)
            {
                return Expression.MakeMemberAccess(expression, member);
        ...
        protected virtual Expression VisitMethodCall(MethodCallExpression methodCallExpression)
        {
            ...
            if (methodCallExpression.Method.Name == "SelectMany" && (methodCallExpression.Method.DeclaringType == typeof(System.Linq.Queryable) || methodCallExpression.Method.DeclaringType == typeof(System.Linq.Enumerable)))
            {
                ...
                {
#if !DeviceDotNet
                    Expression derivedMemberQueryExpression = (memberExpression.Member.GetCustomAttributes(typeof(MetaDataRepository.DerivedMember), false).ToArray()[0] as MetaDataRepository.DerivedMember).Expression;
                    if (OOAdvantech.TypeHelper.GetElementType(derivedMemberQueryExpression.Type) != OOAdvantech.TypeHelper.GetElementType(memberExpression.Type))
                        throw new System.Exception(string.Format("Type mismatch between derived member '{0}.{1}' and derived member query expression", memberExpression.Member.DeclaringType.FullName, memberExpression.Member.Name));
                    methodCallExpression = MergeDerivedMembersQueryExpresionForSelectMany(methodCallExpression);
                    args = methodCallExpression.Arguments;
...
#endif
        /// <MetaDataID>{fa36894a-ee89-4ac2-b0cf-df27bdb6c379}</MetaDataID>
        internal static LambdaExpression GetLambdaExpression(Expression expression)
        {
            if (expression is LambdaExpression)
                return expression as LambdaExpression;
        ...
        private MethodCallExpression GetSelectExpressionWithOrgMethodCallExpressionType(MethodCallExpression orgMethodCallExpression, MethodCallExpression extendedMethodCallExpression)
        {
            ...
            if (GetLambdaExpression(orgResultSelector).Body is NewExpression)
            {
                ...
                Expression selectorResultsBody = Expression.New(selectionResultType.GetMetaData().GetConstructors()[0], selectionResultTypeArguments);
            ...
            System.Reflection.MethodInfo methodInfo = LinqRuntimeTypeBuilder.GetMethod("Select", ref selectionArguments);
        ...
        private MethodCallExpression ExtendSelectManyExpressionWithSelectDerivedMember(MethodCallExpression orgSelectExpression, System.Linq.Expressions.MethodCallExpression derivedMemberQueryExpresion)
        {
            ...
            var selectorBody = Expression.New(selectResultType.GetMetaData().GetConstructors()[0], resultSelector_SourcePropertyInitiator, resultSelector_DerivedMemberInitiator);
            selector = Expression.Quote(Expression.Lambda(selectorBody, sourceParam));
            #endregion
        ...
        /// in other case  returns the original expression.
        /// </returns>
        /// <MetaDataID>{d0b7a2c3-90cf-4151-80c5-b4915f7d2cf0}</MetaDataID>
        private Expression ReplaceParameter(Expression expression, ParameterExpression orgParameter, ParameterExpression newParameter)
        {
            if (expression is UnaryExpression || expression is LambdaExpression)
            {
        ...
        /// in other case returns null
        /// </returns>
        /// <MetaDataID>{ee8721bd-65e9-43c3-8cdf-b48945e568d6}</MetaDataID>
        private Expression ReplaceParameter(Expression expression, ParameterExpression[] parameterExpressions)
        {
            if (expression is MemberExpression)
            {
                foreach (var parameterExpression in parameterExpressions)
                {
                    ...
                    else
                    {
                        Expression newExpression = parameterExpression;
                        List<string> path = GetPath(expression);
                        if (parameterExpression.Type.GetMetaData().GetProperty(path[0]) != null &&
                            parameterExpression.Type.GetMetaData().GetProperty(path[0]).PropertyType == GetParameter(expression).Type.GetMetaData().GetProperty(path[0]).PropertyType)
                        {
                            foreach (var memberName in path)
                                newExpression = Expression.PropertyOrField(newExpression, memberName);
                            return newExpression;
                        ...
                        {
                            path.Insert(0, GetParameter(expression).Name);
                            if (parameterExpression.Type.GetMetaData().GetProperty(GetParameter(expression).Name) != null &&
                                parameterExpression.Type.GetMetaData().GetProperty(GetParameter(expression).Name).PropertyType == GetParameter(expression).Type)
                            {
                                foreach (var memberName in path)
                                    newExpression = Expression.PropertyOrField(newExpression, memberName);
                                return newExpression;
            ...
            if (expression is ParameterExpression)
            {
                foreach (var parameterExpression in parameterExpressions)
                {
                    if (parameterExpression.Type == expression.Type)
                        return parameterExpression;
                    if (parameterExpression.Type.GetMetaData().GetProperty((expression as ParameterExpression).Name) != null &&
                        parameterExpression.Type.GetMetaData().GetProperty((expression as ParameterExpression).Name).PropertyType == expression.Type)
                    {
                        List<string> path = GetPath(expression);
                        path.Insert(0, (expression as ParameterExpression).Name);
                        Expression newExpresion = parameterExpression;
        ...
        private MethodCallExpression ExtendSeletcManyExpressionWithSelectManyDerivedMember(MethodCallExpression orgSelectManyExpression, System.Linq.Expressions.MethodCallExpression derivedMemberQueryExpresion)
        {
            ...
            var resultSelectorBody = Expression.New(selectResultType.GetMetaData().GetConstructors()[0], resultSelector_SourcePropertyInitiator, resultSelector_DerivedMemberInitiator);
        ...
        private MethodCallExpression ExtendSeletcExpressionWithSelectManyDerivedMember(MethodCallExpression orgSelectExpression, System.Linq.Expressions.MethodCallExpression derivedMemberQueryExpresion)
        {
            ...
            Expression selectorBody = Expression.New(selectResultType.GetMetaData().GetConstructors()[0], resultSelector_SourcePropertyInitiator, resultSelector_DerivedMemberInitiator);
            selector = Expression.Quote(Expression.Lambda(selectorBody, sourceParam));
            #endregion
        ...
        /// </param>
        /// <returns></returns>
        /// <MetaDataID>{2e1c1be5-c52f-456c-b859-05cd2fbb57db}</MetaDataID>
        private LambdaExpression GetLambdaExpression(Expression expression, ParameterExpression[] parameterExpressions)
        {
            if (expression is LambdaExpression)
            {
        ...
        private void GetDerivedMemberSourcePath(Type type, Expression derivedMemberSelectorSource, List<string> path)
        {
            foreach (var property in type.GetMetaData().GetProperties())
            {
                if (property.Name.IndexOf("<>h__TransparentIdentifier") == 0 && property.PropertyType.Name.IndexOf("DeriveMember") != property.PropertyType.Name.Length - "DeriveMember".Length - 1)
                {
                    path.Add(property.Name);
        ...
        private System.Linq.Expressions.MemberExpression GetDerividedMemberAccess(Expression selector)
        {
            ...
            if (selector is LambdaExpression)
                selector = (selector as LambdaExpression).Body;
            while ((selector is System.Linq.Expressions.MemberExpression) && !((selector as System.Linq.Expressions.MemberExpression).Member.GetCustomAttributes(typeof(OOAdvantech.MetaDataRepository.DerivedMember), false).ToArray().Length > 0))
                selector = (selector as System.Linq.Expressions.MemberExpression).Expression;

            if (selector is System.Linq.Expressions.MemberExpression && ((selector as System.Linq.Expressions.MemberExpression).Member.GetCustomAttributes(typeof(OOAdvantech.MetaDataRepository.DerivedMember), false).ToArray().Length > 0))
                return selector as System.Linq.Expressions.MemberExpression;
        ...
        private Expression ExtendMainQueryWithDerivedMemberQueryExpression(MethodCallExpression derivedMemberExpresion, Expression orgSource, MemberExpression orgDerivedMemberCollectionSelector)
        {
            ...
            else
            {
                ...
                if ((orgSource is MemberExpression) && (orgSource as MemberExpression).Expression.Type == (derivedExpressionCollectionSelector as MemberExpression).Expression.Type)
                {
                    ...
                    Expression mergeResultsBody = Expression.New(resultType.GetMetaData().GetConstructors()[0], orgDerivedMemberCollectionSelectorOwner, mergeSourceParam);
                    var mergeResults = Expression.Quote(Expression.Lambda(mergeResultsBody, mergeSourceParam));
                ...
                else
                {
                    ...
                    var mergeSelectorParam = ((derivedMemberExpresion.Arguments[2] as UnaryExpression).Operand as LambdaExpression).Parameters[1];
                    resultType = LinqRuntimeTypeBuilder.GetDynamicType(GetLambdaExpression(derivedMemberExpresion.Arguments[2]).Body.Type.Name + "DeriveMember", dynamicTypeParameters);
                    MethodCallExpression extendedExpresion = null;
                    if (!hasTheSameSourceParameterName)
                    {
                        Expression mergeResultsBody = Expression.New(resultType.GetMetaData().GetConstructors()[0], mergeSourceParam, derivedMemberSourceParam, mergeSelectorParam);
                        var mergeResults = Expression.Lambda(mergeResultsBody, mergeSourceParam, mergeSelectorParam);
                        extendedExpresion = LinqRuntimeTypeBuilder.GetSelectManyExprsession(orgSource, Expression.Lambda(mergeSelectorBody, mergeSourceParam), mergeResults);
                    }
                    else
                    {
                        Expression mergeResultsBody = Expression.New(resultType.GetMetaData().GetConstructors()[0], mergeSourceParam, mergeSelectorParam);
                        var mergeResults = Expression.Lambda(mergeResultsBody, mergeSourceParam, mergeSelectorParam);
                        extendedExpresion = LinqRuntimeTypeBuilder.GetSelectManyExprsession(orgSource, Expression.Lambda(mergeSelectorBody, mergeSourceParam), mergeResults);
                    }
...
#endif
        /// <MetaDataID>{71deeaa3-265a-45fc-afe9-bada7bb4ca7d}</MetaDataID>
        private Expression GetMemberOwner(Expression typeMemberAccessExxpresion)
        {
            if (typeMemberAccessExxpresion is MemberExpression)
                return (typeMemberAccessExxpresion as MemberExpression).Expression;
        ...
        /// <MetaDataID>{24e09a43-e56b-4c8b-9a84-2fc19851e891}</MetaDataID>
        private static List<string> GetPath(Expression memberExpression)
        {
            List<string> path = new List<string>();
            while (memberExpression is MemberExpression)
        ...
        /// <MetaDataID>{f02abcf0-b117-4bdf-95de-e286fed78dcf}</MetaDataID>
        protected MethodCallExpression UpdateMethodCall(MethodCallExpression m, Expression obj, System.Reflection.MethodInfo method, IEnumerable<Expression> args)
        {
            if (obj != m.Object || method != m.Method || args != m.Arguments)
            {
                return Expression.Call(obj, method, args);
        ...
        /// <MetaDataID>{66fe01c0-5a19-481d-9965-f2018b9ac892}</MetaDataID>
        protected virtual ReadOnlyCollection<Expression> VisitMemberAndExpressionList(ReadOnlyCollection<System.Reflection.MemberInfo> members, ReadOnlyCollection<Expression> original)
        {
            if (original != null)
            {
                List<Expression> list = null;
        ...
        /// <MetaDataID>{0ec6c548-e8a3-4be5-9703-e3d4c792d644}</MetaDataID>
        protected virtual Expression VisitMemberAndExpression(System.Reflection.MemberInfo member, Expression expression)
        {
            return this.Visit(expression);
        }
        ...
        /// <MetaDataID>{30c9bbdc-dc1a-4431-8ac0-1ef4f60be84b}</MetaDataID>
        protected MemberAssignment UpdateMemberAssignment(MemberAssignment assignment, System.Reflection.MemberInfo member, Expression expression)
        {
            if (expression != assignment.Expression || member != assignment.Member)
            {
                return Expression.Bind(member, expression);
        ...
        /// <MetaDataID>{94105dd0-2201-476b-8b45-3809b3c085ac}</MetaDataID>
        protected MemberMemberBinding UpdateMemberMemberBinding(MemberMemberBinding binding, System.Reflection.MemberInfo member, IEnumerable<MemberBinding> bindings)
        {
            if (bindings != binding.Bindings || member != binding.Member)
            {
                return Expression.MemberBind(member, bindings);
        ...
        /// <MetaDataID>{21715d5e-10d5-49a6-ad14-406acfd0dc26}</MetaDataID>
        protected MemberListBinding UpdateMemberListBinding(MemberListBinding binding, System.Reflection.MemberInfo member, IEnumerable<ElementInit> initializers)
        {
            if (initializers != binding.Initializers || member != binding.Member)
            {
                return Expression.ListBind(member, initializers);
        ...
        /// <MetaDataID>{10759b20-4aa2-4462-837b-7191cad880a4}</MetaDataID>
        protected NewExpression UpdateNew(NewExpression nex, System.Reflection.ConstructorInfo constructor, IEnumerable<Expression> args, IEnumerable<System.Reflection.MemberInfo> members)
        {
            if (args != nex.Arguments || constructor != nex.Constructor || members != nex.Members)
            {
                if (nex.Members != null)
        ...
        /// </summary>
        /// <MetaDataID>{a25005d2-0c25-42fe-b246-1b5dfb9d6d08}</MetaDataID>
        ...
        /// </summary>
        /// <MetaDataID>{21775ff8-d64e-4233-a9bc-aeba9a0b4c30}</MetaDataID>
        ...
        /// <MetaDataID>{dcd7ab8c-472d-49dc-ab76-6a290c871fe7}</MetaDataID>
        ...
        /// <MetaDataID>{b01cd447-f187-4f99-a66c-b8835f2b81c8}</MetaDataID>
        ...
        protected virtual IEnumerable<MemberBinding> VisitBindingList(ReadOnlyCollection<MemberBinding> original, ref  ExpressionTreeNode parent)
        {
            ...
            while (num < count)
            {
                ...
                expresionTreeNode.Alias = item.Member.Name;
        ...
        protected virtual Expression VisitMethodCall(MethodCallExpression methodCallExpression, ref  ExpressionTreeNode parent)
        {
            ...
            else if (methodCallExpression.Method.Name == "Partition" && (methodCallExpression.Method.DeclaringType == typeof(System.Linq.OOAdvantechExtraOperators) || methodCallExpression.Method.DeclaringType == typeof(System.Linq.Queryable) || methodCallExpression.Method.DeclaringType == typeof(System.Linq.Enumerable)))
                expresionTreeNode = CreateExpressionTreeNode(ExpressionTreeNodeType.Partition, methodCallExpression, parent, this);//  new SelectExpressionTreeNode(m, parent, LINQObjectQuery);
            else if (methodCallExpression.Method.Name == "SelectMany" && (methodCallExpression.Method.DeclaringType == typeof(System.Linq.Queryable) || methodCallExpression.Method.DeclaringType == typeof(System.Linq.Enumerable)))
            {
                ...
                expresionTreeNode = CreateExpressionTreeNode(ExpressionTreeNodeType.AggregateFunction, methodCallExpression, parent, this);//  new SelectExpressionTreeNode(m, parent, LINQObjectQuery);
            else if (methodCallExpression.Method.Name == "Contains" && methodCallExpression.Method.DeclaringType.GetMetaData().GetInterface("ICollection`1") != null)
        ...
        protected virtual NewExpression VisitNew(NewExpression nex, ref  ExpressionTreeNode parent)
        {
            ...
            //    ExpressionTreeNode argTreeNode = null;
            //    foreach (var treeNode in expresionTreeNode.Nodes)
            //    {
            //        if (treeNode.Expression == arg)
            //        {
            //            argTreeNode = treeNode;
            //            break;
            //        }
            //        else if(treeNode is QueryExpressions.ParameterExpressionTreeNode)
            //        {
            //            QueryExpressions.MemberAccessExpressionTreeNode member = (treeNode as QueryExpressions.ParameterExpressionTreeNode).MemberAccess;
            //            while (member != null && member.Nodes.Count>0)
            //                member = member.Nodes[0] as QueryExpressions.MemberAccessExpressionTreeNode;
            //            if (member != null && member.Expression == arg)
            //            {
            //                argTreeNode = treeNode;
            //                break;
            //            }
            ...
            //    }
            //    if (argTreeNode != null)
            //        argTreeNode.Alias = nex.Constructor.GetParameters()[i].Name;

            //    //var argTreeNode=  (from treeNode in expresionTreeNode.Nodes
            //    //   where treeNode.Expression == arg
            //    //   select treeNode).FirstOrDefault();
            //    //if (argTreeNode != null)
            //    //    argTreeNode.Alias = (arg as System.Linq.Expressions.ParameterExpression).Name;
            //    i++;
            //}

            foreach (QueryExpressions.ParameterExpressionTreeNode parameter in expresionTreeNode.Nodes.OfType<QueryExpressions.ParameterExpressionTreeNode>())
                parameter.InitCtorParameter = true;
