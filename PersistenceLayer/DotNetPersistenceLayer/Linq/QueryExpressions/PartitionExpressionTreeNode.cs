...
namespace OOAdvantech.Linq.QueryExpressions
{
    ...
    /// <MetaDataID>{1f07dae0-4ab2-40c6-b483-63bd687317ac}</MetaDataID>
    class PartitionExpressionTreeNode : MethodCallAsCollectionProviderExpressionTreeNode
    {
        public PartitionExpressionTreeNode(Expression exp, ExpressionTreeNode parent, Translators.ExpressionVisitor expressionTranslator)
            : base(exp, parent, expressionTranslator)
        {
        }
        internal override DataNode BuildDataNodeTree(OOAdvantech.MetaDataRepository.ObjectQueryLanguage.DataNode dataNode, ILINQObjectQuery linqObjectQuery)
        {
            dataNode = SourceCollection.BuildDataNodeTree(dataNode, linqObjectQuery);
            //  dataNode.OfTypeFilterClassifierFullName =TypeHelper.GetElementType( Expression.Type).FullName;
            DataNode = dataNode;
            ...
            DataNode.HasStorageCellConstrain = true;
            if (this.DataNode.ObjectQuery is ObjectsContextQuery && (this.DataNode.ObjectQuery as ObjectsContextQuery).ObjectStorage != null)
            {
                string storageCellIdentity = (this.DataNode.ObjectQuery as ObjectsContextQuery).ObjectStorage.StorageMetaData.StorageIdentity + "/" + (Nodes[1] as ConstantExpressionTreeNode).Value;
                this.DataNode.StorageCellIDsConstrain.Add(storageCellIdentity);
            }
        ...
        public override void BuildDataFilter()
        {
            ...
            //searchFactor.Criterion = new Criterion(Criterion.ComparisonType.StrorageCellsFilter, comparisonTerms, ExpressionTranslator.LINQObjectQuery, true, searchFactor);
