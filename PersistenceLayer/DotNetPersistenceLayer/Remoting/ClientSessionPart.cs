namespace OOAdvantech.Remoting
{
    ...
    public class ClientSessionPart : MarshalByRefObject, IWeakRefernceEventHandler, IRemoteEventHandler
    {
        ...
        {
            if (EventSubscriptions.ContainsKey(proxy.ObjectUri))
            {
                EventInfoData eventInfoData = EventSubscriptions[proxy.ObjectUri].Where(x => x.EventInfo == eventInfo.EventInfo).FirstOrDefault();
                if (eventInfoData != null)
                    EventSubscriptions[proxy.ObjectUri].Remove(eventInfoData);
