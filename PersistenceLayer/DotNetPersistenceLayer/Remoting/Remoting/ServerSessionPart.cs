    ...
    public class ServerSessionPart : MarshalByRefObject, IWeakRefernceEventHandler, IServerSessionPart
    {
            ...
            lock (EventConsumers)
            {
                foreach (System.Collections.Generic.KeyValuePair<string, System.Collections.Generic.Dictionary<EventInfo, EventConsumerProxy>> entry in EventConsumers)
                {
                    foreach (EventConsumerProxy eventConsumer in entry.Value.Values)
                    {
                        ...
                        {
                            if (!pendingEvents.ContainsKey(eventConsumer.EventPublisherUri.TransientUri))
                                pendingEvents[eventConsumer.EventPublisherUri.TransientUri] = new OOAdvantech.Collections.Generic.Dictionary<EventInfoData, System.Collections.Generic.List<object>>();

                            System.Collections.Generic.List<object> marshaledArgs = eventConsumer.PendingEventArgs.ToList();
                            pendingEvents[eventConsumer.EventPublisherUri.TransientUri].Add(new EventInfoData(eventConsumer.EventInfo), marshaledArgs);
        ...
        public void Unsubscribe(ExtObjectUri eventPublisherUri, EventInfoData eventInfoData)
        {
            ...
            lock (EventConsumers)
            {
                ...
                if (EventConsumers.ContainsKey(eventPublisherUri.TransientUri) && EventConsumers[eventPublisherUri.TransientUri].ContainsKey(eventInfo))
                {
                    EventConsumerProxy eventConsumerProxy = EventConsumers[eventPublisherUri.TransientUri][eventInfo];
                    EventConsumers[eventPublisherUri.TransientUri].Remove(eventInfo);
                    if (EventConsumers[eventPublisherUri.TransientUri].Count == 0)
                        EventConsumers.Remove(eventPublisherUri.TransientUri);
        ...
        public void Subscribe(ExtObjectUri eventPublisherUri, EventInfoData eventInfoData, IRemoteEventHandler clientSessionPart)
        {
            ...
            lock (EventConsumers)
            {
                ...
                if (!EventConsumers.ContainsKey(eventPublisherUri.TransientUri))
                    EventConsumers[eventPublisherUri.TransientUri] = new System.Collections.Generic.Dictionary<EventInfo, EventConsumerProxy>();
                //Creates (if doesn't exist) an EventConsumers for event of eventInfo. 
                if (!EventConsumers[eventPublisherUri.TransientUri].ContainsKey(eventInfo))
                {
                    EventConsumers[eventPublisherUri.TransientUri][eventInfo] = new EventConsumerProxy(eventPublisherUri, eventPublisherObject, eventInfo, this, clientSessionPart);
                }
                else
                {
                    EventConsumerProxy eventConsumerProxy = EventConsumers[eventPublisherUri.TransientUri][eventInfo];
