namespace OOAdvantech.MySQLPersistenceRunTime.ObjectQueryLanguage
{
    ...
    /// <MetaDataID>{7d1de641-61b3-4df0-9235-b32a2dd69a62}</MetaDataID>
    public class DataLoader : PersistenceLayerRunTime.StorageDataLoader
    {
        public override bool CanAggregateFanctionsRosolvedLocally
        {
        ...
        public override System.Collections.Generic.List<string> RecursiveParentReferenceColumns
        {
            get
            {
                ...
                else
                {
                    ...
                    RDBMSMetaDataRepository.AssociationEnd associationEnd = (LoadFromStorage  as RDBMSMetaDataRepository.Storage).GetEquivalentMetaObject(DataNode.AssignedMetaObject.Identity.ToString(), typeof(MetaDataRepository.AssociationEnd)) as RDBMSMetaDataRepository.AssociationEnd;
        ...
        public OOAdvantech.PersistenceLayer.ObjectStorage ObjectStorage
        {
            get
            {
               // StorageProvider storageProvider = new StorageProvider();
                return PersistenceLayer.ObjectStorage.OpenStorage(LoadFromStorage.StorageName, LoadFromStorage.StorageLocation, LoadFromStorage.StorageType);
            }
        }
            ...
            get
            {
                if (_LoadFromStorage == null)
                {
                    foreach (MetaDataRepository.StorageCell storageCell in DataLoaderMetadata.StorageCells)
                    {
                        ...
                        _LoadFromStorage = storageCell.Namespace as RDBMSMetaDataRepository.Storage;
                        break;
                    }
                }
                ...
                lock (ObjectStorage)
                {
                    ...
                    foreach (DataNode memberDataNode in DataNode.SubDataNodes)
                    {
                        if (memberDataNode.Type == DataNode.DataNodeType.Count && GlobalResolveCriterions.Count == 0)
                        {
                            ...
                            if (memberDataNode.AggregateExpressionDataNodes == null)
                            {
                                singleRow = true;
                            ...
                            else
                            {
                                ...
                                DataNode parentDataNode = memberDataNode.AggregateExpressionDataNodes;
                                ...
                                if (memberDataNode.AggregateExpressionDataNodes != null && memberDataNode.AggregateExpressionDataNodes.SearchCondition != null)
                                    whereClauseSQLQuery += "and (" + GetSQLFilterStatament(memberDataNode.AggregateExpressionDataNodes.SearchCondition) + ")";
        ...
        public override void RetrieveFromStorage()
        {
            ...
            if (!DataNode.ParticipateInSelectClause && !DataNode.DataNodeMembersParticipateInSelectClause)
            {
                ...
                foreach (Criterion criterion in DataNode.BranchSearchCriterions)
                {
            ...
            System.Data.Common.DbConnection connection = (PersistenceLayer.ObjectStorage.GetStorageOfObject(LoadFromStorage.Properties) as MySQLMetaDataPersistenceRunTime.ObjectStorage).Connection ;
            lock (connection)
            {
...
////                string objectQuery = @"#OQL: SELECT morder, morder.Name OrderName ,  morder.OrderDetails.Name OrderDetailName 
////                                 FROM AbstractionsAndPersistency.IOrder morder  WHERE morder.Name <> 'morder.Client.Name'   OR (morder.OrderDetails.Price.Product.Name  = 'sprite' AND morder.OrderDetails.Price.Product.Name ='kes') #";
                ...
                foreach (DataNode subDataNode in DataNode.SubDataNodes)
                {
                    ...
                    {
                        RDBMSMetaDataRepository.AssociationEnd associationEnd = (LoadFromStorage  as RDBMSMetaDataRepository.Storage).GetEquivalentMetaObject(subDataNode.AssignedMetaObject.Identity.ToString(), typeof(MetaDataRepository.AssociationEnd)) as RDBMSMetaDataRepository.AssociationEnd;
                        ...
                        //if (RelatedDataNodes.ContainsKey(subDataNode))
                        {
                            ...
                            if ((subDataNode.AssignedMetaObject as MetaDataRepository.AssociationEnd).Association.Specializations.Count == 0)
                            {
                                ...
                                command.CommandText =BuildManyToManyTablesJoin(subDataNode, true, null, RelatedDataNodes, true);
                            }
                            else
                            {
                                command = connection.CreateCommand();
                                command.CommandText =BuildAbstractAssociationTablesJoin(subDataNode, true, null, RelatedDataNodes, true);
                            }
                        ...
                        AssotiationTableRelationshipData relationshipData = new AssotiationTableRelationshipData(parentRelationColumns, childRelationColumns, ManyToManyRelationData);
        ...
        private void TransferCollectionsDataToServer()
        {
            foreach (MetaDataRepository.ObjectQueryLanguage.Criterion criterion in LocalResolvedCriterions.Keys)
            {
                if (criterion.CriterionType == ComparisonTermsType.CollectionContainsAnyAll)
                {
                    ...
                    DataObjects.DataBase dataBase = new DataObjects.DataBase(LoadFromStorage as RDBMSMetaDataRepository.Storage, "tmp");
                    OOAdvantech.RDBMSMetaDataRepository.Table table = new OOAdvantech.RDBMSMetaDataRepository.Table();
                    table.Name = parameterName;
                    System.Data.SqlClient.SqlConnection connection = (PersistenceLayer.ObjectStorage.GetStorageOfObject(LoadFromStorage.Properties) as MySQLMetaDataPersistenceRunTime.ObjectStorage).Connection as System.Data.SqlClient.SqlConnection;
                    ...
                    {
                        RDBMSMetaDataRepository.Attribute attribute = (LoadFromStorage  as RDBMSMetaDataRepository.Storage).GetEquivalentMetaObject(criterion.CollectionContainsDataNode.AssignedMetaObject) as RDBMSMetaDataRepository.Attribute;
                    ...
                    else
                    {
                        ...
                        foreach (RDBMSMetaDataRepository.Column column in ((LoadFromStorage  as RDBMSMetaDataRepository.Storage).GetEquivalentMetaObject(criterion.CollectionContainsDataNode.Classifier) as RDBMSMetaDataRepository.MappedClassifier).ObjectIDColumns)
                        ...
                        foreach (object value in criterion.ParameterValue as System.Collections.IEnumerable)
                        {
                            ...
                            System.Data.DataRow dataRow = dataTable.NewRow();
                            foreach (RDBMSMetaDataRepository.IdentityColumn column in ((LoadFromStorage  as RDBMSMetaDataRepository.Storage).GetEquivalentMetaObject(criterion.CollectionContainsDataNode.Classifier) as RDBMSMetaDataRepository.MappedClassifier).ObjectIDColumns)
        ...
        private string GetContainsAnyAllSQL(MetaDataRepository.ObjectQueryLanguage.Criterion criterion)
        {
            if (criterion.ComparisonOperator == Criterion.ComparisonType.ContainsAny)
            {
                ...
                if (criterion.CollectionContainsDataNode.Type == DataNode.DataNodeType.OjectAttribute)
                {
                    if (criterion.CollectionContainsDataNode.ParentDataNode.DataSource.DataLoaders.ContainsKey(LoadFromStorage.StorageIdentity))
                    {
                        query = selectionList + " FROM " + query + "INNER JOIN #" + (criterion.ComparisonTerms[1] as ParameterComparisonTerm).ParameterName + " on ";
                        query += "`" + criterion.CollectionContainsDataNode.ParentDataNode.Alias + "`.`" + criterion.CollectionContainsDataNode.Name + "` = #" + (criterion.ComparisonTerms[1] as ParameterComparisonTerm).ParameterName + ".`" + criterion.CollectionContainsDataNode.Name + "`";
                    }
                    else
                    {
                ...
                else
                {
                    if (criterion.CollectionContainsDataNode.ParentDataNode.DataSource.DataLoaders.ContainsKey(LoadFromStorage.StorageIdentity))
                    {
                        ...
                        foreach (RDBMSMetaDataRepository.Column column in (criterion.CollectionContainsDataNode.DataSource.DataLoaders[LoadFromStorage.StorageIdentity].Classifier as RDBMSMetaDataRepository.MappedClassifier).ObjectIDColumns)
                        {
                            if (joinCriterion != null)
                                joinCriterion += " AND ";
                            joinCriterion += "`" + criterion.CollectionContainsDataNode.Alias + "`.`" + column.Name + "` = #" + (criterion.ComparisonTerms[1] as ParameterComparisonTerm).ParameterName + ".`" + column.Name + "`";
                        }
                        query += joinCriterion;
                    }
                    {
            ...
            else
            {
                ...
                if (criterion.CollectionContainsDataNode.Type == DataNode.DataNodeType.OjectAttribute)
                {
                    if (criterion.CollectionContainsDataNode.ParentDataNode.DataSource.DataLoaders.ContainsKey(LoadFromStorage.StorageIdentity))
                    {
                        selectionList += ",`" + criterion.CollectionContainsDataNode.ParentDataNode.Alias + "`.`" + criterion.CollectionContainsDataNode.Name + "` as [FilteredWith" + (criterion.ComparisonTerms[1] as ParameterComparisonTerm).ParameterName + "`";
                        query = selectionList + " FROM " + query + "INNER JOIN #" + (criterion.ComparisonTerms[1] as ParameterComparisonTerm).ParameterName + " ON ";
                        query += "`" + criterion.CollectionContainsDataNode.ParentDataNode.Alias + "`.`" + criterion.CollectionContainsDataNode.Name + "` = #" + (criterion.ComparisonTerms[1] as ParameterComparisonTerm).ParameterName + ".`" + criterion.CollectionContainsDataNode.Name + "`";
                    }
                    else
                ...
                else
                {
                    ...
                    {
                        foreach (RDBMSMetaDataRepository.Column column in ((LoadFromStorage  as RDBMSMetaDataRepository.Storage).GetEquivalentMetaObject(criterion.CollectionContainsDataNode.Classifier) as RDBMSMetaDataRepository.MappedClassifier).ObjectIDColumns)
                            selectionList += ",`" + criterion.CollectionContainsDataNode.Alias + "`.`" + column.Name + "` as [FilteredWith" + (criterion.ComparisonTerms[1] as ParameterComparisonTerm).ParameterName + "`";
                ...
                case ComparisonTermsType.LiteralWithObjectAttribute:
                    {
                        ...
                        if (criterion.ComparisonOperator == Criterion.ComparisonType.Like)
                            ...
                            literalValue = (ObjectStorage as ObjectStorage).TypeDictionary.ConvertToSQLString(literalComparisonTerm.Value);
                            ...
                            (criterion.IsNotNULL || criterion.IsNotNULL)))
                        {
                            ...
                            {
                                RDBMSMetaDataRepository.Attribute attribute = (LoadFromStorage  as RDBMSMetaDataRepository.Storage).GetEquivalentMetaObject(objectAttributeComparisonTerm.DataNode.ParentDataNode.AssignedMetaObject.Identity.ToString(), typeof(MetaDataRepository.Attribute)) as RDBMSMetaDataRepository.Attribute;
                ...
                case ComparisonTermsType.ParameterWithObjectAttribute:
                    {
                        ...
                        if (criterion.ComparisonOperator == Criterion.ComparisonType.Like)
                            ...
                            parameterValue = (ObjectStorage as ObjectStorage).TypeDictionary.ConvertToSQLString(parameterComparisonTerm.ParameterValue);
            ...
            get
            {
                ...
                foreach (MetaDataRepository.ObjectQueryLanguage.DataNode dataNode in DataNode.SubDataNodes)
                {
                    if (dataNode.AssignedMetaObject is MetaDataRepository.AssociationEnd)
                    {
                        ...
                        {
                            RDBMSMetaDataRepository.Attribute attribute = (LoadFromStorage  as RDBMSMetaDataRepository.Storage).GetEquivalentMetaObject(DataNode.AssignedMetaObject.Identity.ToString(), typeof(MetaDataRepository.Attribute)) as RDBMSMetaDataRepository.Attribute;
        ...
        {
            RDBMSMetaDataRepository.MappedClassifier mappedClassifier = (storage  as RDBMSMetaDataRepository.Storage).GetEquivalentMetaObject(classifier.Identity.ToString(), typeof(MetaDataRepository.Classifier)) as RDBMSMetaDataRepository.MappedClassifier;
            ...
            foreach (RDBMSMetaDataRepository.Column column in mappedClassifier.TypeView.ViewColumns)
            {
                ...
                if (column.Type != null)
                    nullValue = (PersistenceLayerRunTime.ObjectStorage.GetStorageOfObject(storage) as RDBMSMetaDataPersistenceRunTime.AdoNetObjectStorage).TypeDictionary.GetDBNullScript(column.Type.FullName);
        ...
        string BuildSQLStatament()
        {
            ...
            else
            {
                ...
                foreach (RDBMSMetaDataRepository.Column column in columns)
                {
                    string nullValue = "NULL";
                    if (column.Type != null)
                        nullValue = (ObjectStorage as ObjectStorage).TypeDictionary.GetDBNullScript(column.Type.FullName);
        ...
        private string BuildSelectQuery(System.Collections.ArrayList ColumnsNames, RDBMSMetaDataRepository.View view, RDBMSMetaDataRepository.StorageCell storageCell)
        {
            ...
            string StorageName ="`"+LoadFromStorage.StorageName+"`.";
            ...
            if (storageCell != null)
                FromClause = "\nFROM ( SELECT *," + DataLoaderMetadata.QueryStorageID + " as StorageIdentity  FROM " + StorageName + ViewName + ") " +  ViewName;// + ViewAlias;
            else
        ...
        string BuildRecursiveLoadManyToMany(string loadDataSQLQuery, System.Collections.Generic.Dictionary<DataNode, bool> relatedDataNodes)
        {
            ...
            MetaDataRepository.Classifier classifier = (LoadFromStorage  as RDBMSMetaDataRepository.Storage).GetEquivalentMetaObject(DataNode.Classifier) as MetaDataRepository.Classifier;
                                                ...
                                                out string joinWhereAttributes)
        {
                ...
                (relatedDataNode == DataNode.ParentDataNode && DataNode.AssignedMetaObject is MetaDataRepository.AssociationEnd))
            {
                ...
                if (relatedDataNode == DataNode.ParentDataNode)
                    associationEnd = (LoadFromStorage  as RDBMSMetaDataRepository.Storage).GetEquivalentMetaObject(DataNode.AssignedMetaObject.Identity.ToString(), typeof(MetaDataRepository.AssociationEnd)) as RDBMSMetaDataRepository.AssociationEnd;
                else
                    associationEnd = (LoadFromStorage  as RDBMSMetaDataRepository.Storage).GetEquivalentMetaObject(relatedDataNode.AssignedMetaObject.Identity.ToString(), typeof(MetaDataRepository.AssociationEnd)) as RDBMSMetaDataRepository.AssociationEnd;
                ...
                if (associationEnd == associationEndWithReferenceColumns)
                {
                    ...
                    if (relatedDataNode == DataNode.ParentDataNode)
                    {
                        ...
                        {
                            RDBMSMetaDataRepository.Attribute attribute = (LoadFromStorage  as RDBMSMetaDataRepository.Storage).GetEquivalentMetaObject(relatedDataNode.AssignedMetaObject.Identity.ToString(), typeof(MetaDataRepository.Attribute)) as RDBMSMetaDataRepository.Attribute;
                ...
                if (subDataNodeDataLoader != null)
                {
                    ...
                    FromClauseSQLQuery += GetEmptyDataSQLStatament(relatedDataNode.Classifier, LoadFromStorage as RDBMSPersistenceRunTime.Storage) + " AS `" + relatedDataNode.Alias + "` ";//ON ";
