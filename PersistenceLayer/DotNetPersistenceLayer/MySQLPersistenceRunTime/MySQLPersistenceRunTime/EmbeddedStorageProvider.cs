...
namespace OOAdvantech.MySQLPersistenceRunTime
{
    ...
    /// <MetaDataID>{5f07099d-ffc3-434f-8313-6ec328d36bf0}</MetaDataID>
            ...
            lock (OpenStorageLock)
            {
                try
                {
                    ...
                    using (Transactions.SystemStateTransition stateTransition = new OOAdvantech.Transactions.SystemStateTransition())
                    {
                        ...
                        PersistenceLayer.ObjectStorage metaDataObjectStorage = PersistenceLayer.ObjectStorage.OpenStorage(StorageName, StorageLocation, "OOAdvantech.MySQLMetaDataPersistenceRunTime.EmbeddedStorageProvider");
                ...
                using (System.Transactions.TransactionScope transactionScope = new System.Transactions.TransactionScope(Transactions.TransactionInterop.GetSystemTransaction(Transactions.Transaction.Current) as System.Transactions.Transaction))
                {
                    ...
                    try
                    {
                        PersistenceLayer.ObjectStorage MetaDataStorageSession = ObjectStorage.NewStorage(storageName,
                            storageLocation,
                            "OOAdvantech.MySQLMetaDataPersistenceRunTime.EmbeddedStorageProvider", true);
                        ...
                        RDBMSPersistenceRunTime.Storage mObjectStorage = new RDBMSPersistenceRunTime.Storage(storageName, storageLocation, "OOAdvantech.MySQLPersistenceRunTime.EmbeddedStorageProvider", GetDataBase(sqlServerInstanceName, storageName));
