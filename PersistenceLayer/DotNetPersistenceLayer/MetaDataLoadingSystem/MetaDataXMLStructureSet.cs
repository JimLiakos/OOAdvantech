...
{
	/// <MetaDataID>{EDF6A9E7-ED35-4120-A3A1-9C385358E077}</MetaDataID>
    public class MetaDataStructureSet : Collections.StructureSet
	{
		...
		protected void LoadMembers()
		{
			...
			if(Members==null)
				Members=new MetaDataMemberList();

			((MetaDataMemberList)Members).AddMember(NodeMemberAlias);
            Collections.Member aMember = Members[NodeMemberAlias];
			...
			StorageInstanceRef=(MetaDataStorageInstanceRef)((PersistenceLayerRunTime.ObjectStorage)SourceStorageSession).CreateStorageInstanceRef(NewObject,ObjID);
			//StorageInstanceRef.ObjectID=ObjID;
			((MetaDataStorageInstanceRef)StorageInstanceRef).TheStorageIstance=CurrXmlElement;
		...
		/*
		/// <MetaDataID>{760598EC-AD7F-4FCA-B7F9-45834BFA9241}</MetaDataID>
		public override void MoveLast()
		{
			if(XmlNodes.Count!=0)
				Index=XmlNodes.Count-1;
		}
		*/
		...
		public void Open(string Query)
		{
			...
			System.Xml.XmlDocument XMLDocumentData=((MetaDataStorageSession)SourceStorageSession).XMLDocument;
			...
			if(mClass==null)
			{
                MetaDataRepository.Component AssemblyMetaObject = DotNetMetaDataRepository.MetaObjectMapper.FindMetaObjectFor(mType.Assembly) as MetaDataRepository.Component;
                if (AssemblyMetaObject == null)
                {
                    DotNetMetaDataRepository.Assembly assembly = new DotNetMetaDataRepository.Assembly(mType.Assembly);

                    DotNetMetaDataRepository.MetaObjectMapper.AddTypeMap(mType.Assembly, assembly);
