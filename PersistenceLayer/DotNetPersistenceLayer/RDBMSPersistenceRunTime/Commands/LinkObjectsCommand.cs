...
namespace OOAdvantech.RDBMSPersistenceRunTime.Commands
{
    ...
    public class LinkObjectsCommand : PersistenceLayerRunTime.Commands.LinkObjectsCommand
    {
        ...
        private void LinkOnetoManyOneToOne(RDBMSMetaDataRepository.AssociationEnd associationEnd)
        {
            ...
            var oleDbConnection = (recordOwnerObject.ObjectStorage .StorageMetaData as RDBMSPersistenceRunTime.Storage).StorageDataBase.Connection;
        ...
        private void UpdateMappingDataIfNeeded()
        {
            ...
            if (Association.LinkClass != null)
            {
                ...
                if (!objectCollectionsLink.AssotiationClassStorageCells.Contains(((StorageInstanceRef)RelationObject.RealStorageInstanceRef).StorageInstanceSet))
                {
                    ...
                    OOAdvantech.RDBMSPersistenceRunTime.Commands.UpdateStorageSchema updateStorageSchema = null;
                    if (!OwnerTransactiont.EnlistedCommands.ContainsKey(OOAdvantech.RDBMSPersistenceRunTime.Commands.UpdateStorageSchema.GetIdentity(ObjectStorage)))
                    {
                        updateStorageSchema = new UpdateStorageSchema(RoleA.ObjectStorage as RDBMSPersistenceRunTime.ObjectStorage);
                        OwnerTransactiont.EnlistCommand(updateStorageSchema);
                    }
                    else
                        updateStorageSchema =OwnerTransactiont.EnlistedCommands[(OOAdvantech.RDBMSPersistenceRunTime.Commands.UpdateStorageSchema.GetIdentity(ObjectStorage))] as OOAdvantech.RDBMSPersistenceRunTime.Commands.UpdateStorageSchema;
            ...
            else if (StorageInstanceRef.GetStorageInstanceRef(objectCollectionsLink.Properties).PersistentObjectID == null)
            {
                ...
                if (!OwnerTransactiont.EnlistedCommands.ContainsKey(OOAdvantech.RDBMSPersistenceRunTime.Commands.UpdateStorageSchema.GetIdentity(ObjectStorage)))
                {
                    updateStorageSchema = new UpdateStorageSchema(ObjectStorage as RDBMSPersistenceRunTime.ObjectStorage);
                    OwnerTransactiont.EnlistCommand(updateStorageSchema);
                }
                else
                    updateStorageSchema = OwnerTransactiont.EnlistedCommands[(OOAdvantech.RDBMSPersistenceRunTime.Commands.UpdateStorageSchema.GetIdentity(ObjectStorage))] as OOAdvantech.RDBMSPersistenceRunTime.Commands.UpdateStorageSchema;
