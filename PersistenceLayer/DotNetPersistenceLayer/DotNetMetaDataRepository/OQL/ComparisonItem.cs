	...
	public abstract class ComparisonTerm
	{
		...
		private static ComparisonTermType GetType(Parser.ParserNode comparisonTerm, OQLStatement oqlStatement)
		{
			...
			if(TypeName=="Path")
			{
				...
				{
					if(typeof(MetaDataRepository.Attribute).IsInstanceOfType(PathCorespondingObjectCollection.AssignedMetaObject))
						return ComparisonTermType.ObjectAttribute;
					if(typeof(MetaDataRepository.Class).IsInstanceOfType(PathCorespondingObjectCollection.AssignedMetaObject))
						return ComparisonTermType.Object;
					if(typeof(MetaDataRepository.AssociationEnd).IsInstanceOfType(PathCorespondingObjectCollection.AssignedMetaObject))
					{
