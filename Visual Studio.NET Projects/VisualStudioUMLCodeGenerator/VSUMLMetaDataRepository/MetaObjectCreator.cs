...
namespace OOAdvantech.VSUMLMetaDataRepository
{
    ...
    public class MetaObjectsStack : OOAdvantech.MetaDataRepository.MetaObjectsStack
    {
        ...
        public override OOAdvantech.MetaDataRepository.MetaObject CreateMetaObjectInPlace(OOAdvantech.MetaDataRepository.MetaObject OriginMetaObject, OOAdvantech.MetaDataRepository.MetaObject placeIdentifier)
        {
            ...
            {
                NewMetaObject = new Class((placeIdentifier as IVSUMLModelItemWrapper).UMLModel);
                goto End;
            }


            if (OriginMetaObject is OOAdvantech.MetaDataRepository.Interface)
            {
                NewMetaObject = new Interface((placeIdentifier as IVSUMLModelItemWrapper).UMLModel);
                goto End;
            }
            if (OriginMetaObject is OOAdvantech.MetaDataRepository.Structure)
            {
                NewMetaObject = new Structure((placeIdentifier as IVSUMLModelItemWrapper).UMLModel);
                goto End;
            }
            ...
            {
                NewMetaObject = new Operation((placeIdentifier as IVSUMLModelItemWrapper).UMLModel);
                goto End;
            }
            if (OriginMetaObject is OOAdvantech.MetaDataRepository.Attribute)
            {
                NewMetaObject = new Attribute((placeIdentifier as IVSUMLModelItemWrapper).UMLModel);
                goto End;
            }
            if (OriginMetaObject is OOAdvantech.MetaDataRepository.AttributeRealization)
            {
                NewMetaObject = new AttributeRealization((placeIdentifier as IVSUMLModelItemWrapper).UMLModel);
                    goto End;
                }
            if (OriginMetaObject is OOAdvantech.MetaDataRepository.Method)
            {
                NewMetaObject = new Method((placeIdentifier as IVSUMLModelItemWrapper).UMLModel);
                goto End;
            }
            ...
            {
                NewMetaObject = new Package((placeIdentifier as IVSUMLModelItemWrapper).UMLModel);
                goto End;
        ...
        //    }
            if (OriginMetaObject is OOAdvantech.MetaDataRepository.AssociationEnd)
            {
                OOAdvantech.MetaDataRepository.AssociationEnd originAssociationEnd = OriginMetaObject as OOAdvantech.MetaDataRepository.AssociationEnd;
                    ...
                    IClassifier roseClientClass = null, roseSupplierClass = null;
                ...
                if (clientClassifier is Interface)
                    roseClientClass = (clientClassifier as Interface).VSUmlInterface;
                else if (clientClassifier is Structure)
                    roseClientClass = (clientClassifier as Structure).VSUmlStructure;
                else
                    roseClientClass = (clientClassifier as Class).VSUmlClass;

                if (supplierClassifier is Interface)
                    roseSupplierClass = (supplierClassifier as Interface).VSUmlInterface;
                else
                    roseSupplierClass = (supplierClassifier as Class).VSUmlClass;

                    if (roseClientClass == null || roseSupplierClass == null)
                    return null;
                NewMetaObject = new AssociationEnd((placeIdentifier as IVSUMLModelItemWrapper).UMLModel);
                goto End;
            }
