...
namespace OOAdvantech.VSUMLMetaDataRepository
{
    ...
    public class Structure : OOAdvantech.MetaDataRepository.Structure, IVSUMLModelItemWrapper
    {
        ...
        internal IClass VSUmlStructure;

        public Structure(Microsoft.VisualStudio.Uml.AuxiliaryConstructs.IModel iModel)
        {
            VSUmlModel = iModel;
        }
        ...
        public override void Synchronize(MetaDataRepository.MetaObject originMetaObject)
        {
            ...
            if (VSUmlStructure == null)
            {
                ...
                VSUmlStructure = (_Namespace.Value as Package).VSUmlPackage.CreateClass();
                VSUmlStructure.Name = originMetaObject.Name;
                 
                var stereotype = (from applicableStereotype in (VSUmlStructure as Microsoft.VisualStudio.Uml.Classes.IElement).ApplicableStereotypes
                                  where applicableStereotype.Profile.Name == "OOAdvantechProfile" && applicableStereotype.Name == "struct"
                                  select applicableStereotype).FirstOrDefault();

                if(stereotype != null) 
                    VSUmlStructure.ApplyStereotype(stereotype);
 
                MetaObjectMapper.AddTypeMap(VSUmlStructure as ModelElement, this);
