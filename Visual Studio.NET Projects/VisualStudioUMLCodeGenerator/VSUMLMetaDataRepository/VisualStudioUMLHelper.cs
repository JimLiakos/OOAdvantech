...
namespace OOAdvantech.VSUMLMetaDataRepository
{
      ...
      /// <param name="e"></param>
        ...
        private static void ModelElementDeleted(object sender, ElementDeletedEventArgs e)
        {
            ...
            if (e.ModelElement is IOperation && e.ModelElement.IsDeleted)
            {
                ...
                MetaObjectMapper.RemoveMappedElement(e.ModelElement);
            }
            if (e.ModelElement is IProperty && e.ModelElement.IsDeleted)
            {
                ...
                MetaDataRepository.AssociationEnd associationEnd = MetaObjectMapper.FindMetaObjectFor(e.ModelElement) as MetaDataRepository.AssociationEnd;
                ...
                {
                    MetaDataRepository.Classifier roleAClassifier = associationEnd.Association.RoleA.Specification ;
                    MetaDataRepository.Classifier roleBClassifier = associationEnd.Association.RoleB.Specification ;
                    roleAClassifier.RemoveAssociation( associationEnd.Association);
                    roleBClassifier.RemoveAssociation(associationEnd.Association);
        ...
        public static List<Package> GetPackages(this Microsoft.VisualStudio.Uml.AuxiliaryConstructs.IModel iModel)
        {
            return (from packageElement in iModel.PackagedElements
                    where packageElement.OwningPackage != null
                    select VisualStudioUMLHelper.GetPackageFor(packageElement.OwningPackage,iModel)).ToList();
        }
        ...
        internal static string GetStereotypePropertyValue(this Microsoft.VisualStudio.Modeling.ModelElement modelElement, string propertyName)
        {
            ...
            {
                var stereotypes = (from appliedStereotype in (modelElement as Microsoft.VisualStudio.Uml.Classes.IElement).ApplicableStereotypes
                                                                              where appliedStereotype.Profile.Name == "OOAdvantechProfile"
                                                                              select appliedStereotype).ToArray();
                if (stereotypes.Length == 1)
                    stereotypeInstance = (modelElement as Microsoft.VisualStudio.Uml.Classes.IElement).ApplyStereotype(stereotypes[0]);
                else
                {
                    if (MetaObjectMapper.MetaObjectDictionary.ContainsKey(modelElement) && MetaObjectMapper.MetaObjectDictionary[modelElement] is Class)
                    {
                        foreach (var stereotype in stereotypes)
                        {
                            if (stereotype.Name == "class")
                            {
                                (modelElement as Microsoft.VisualStudio.Uml.Classes.IElement).ApplyStereotype(stereotype);
                                break;
                            }
                        }
                    }
                    ...
                    if (MetaObjectMapper.MetaObjectDictionary.ContainsKey(modelElement) && MetaObjectMapper.MetaObjectDictionary[modelElement] is Component)
                    {
                        foreach (var stereotype in stereotypes)
                        {
                            if (stereotype.Name == "component")
                            {
                                (modelElement as Microsoft.VisualStudio.Uml.Classes.IElement).ApplyStereotype(stereotype);
                                break;
                            }
        ...
        public static Association GetAssociationFor(IAssociation vsUMLAssociation, Microsoft.VisualStudio.Uml.AuxiliaryConstructs.IModel iModel)
        {
            ...
            if (association == null)
            {
                ...
                string metaObjectID = (vsUMLAssociation as ModelElement).GetExtensionData().GetPropertyValue("MetaData", "MetaObjectID");
                if (string.IsNullOrWhiteSpace(metaObjectID))
                {
                    metaObjectID = vsUMLAssociation.GetId().ToString();
                    (vsUMLAssociation as ModelElement).GetExtensionData().SetPropertyValue("MetaData", "MetaObjectID", metaObjectID);
                }
                return new Association(vsUMLAssociation, roleA, roleB, iModel, metaObjectID);
            }
            else
                return association;
        ...
        public static MetaDataRepository.Classifier GetClassifierFor(IClassifier vsUmlClassifier, Microsoft.VisualStudio.Uml.AuxiliaryConstructs.IModel iModel)
        {
            ...
            if (_classifier == null)
            {
                if (vsUmlClassifier is IClass)
                {
                    ...
                    {
                        var applicableStereotype = (from appliedStereotype in (vsUmlClassifier as Microsoft.VisualStudio.Uml.Classes.IElement).ApplicableStereotypes
                                                    where appliedStereotype.Profile.Name == "OOAdvantechProfile" && appliedStereotype.Name == "class"
                                                    select appliedStereotype).FirstOrDefault();
                        _classifier = new Class(vsUmlClassifier as IClass, iModel);
                    }
                }
        ...
        internal static Package GetPackage(this Microsoft.VisualStudio.Uml.AuxiliaryConstructs.IModel iModel, string fullName)
        {
            return (from package in iModel.GetPackages()
                    where package.FullName == fullName
    ...
    public class ExtensionData
    {
        ...
        {
            if (modelElement is IClassifier)
            {
                IPackage package = (modelElement as IClassifier).Namespace as IPackage;
                if (package != null)
                {
                    while (package.OwningPackage != null)
