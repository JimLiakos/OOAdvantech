...
namespace OOAdvantech.VSUMLMetaDataRepository
{
    ...
    public static class VisualStudioUMLHelper
    {
        ...
        static Microsoft.VisualStudio.Modeling.Store ModelStore;
        public static void SubscribeModelEventsConsumer(Microsoft.VisualStudio.Modeling.Store store)
        {
            if (ModelStore != store)
            {
                ModelStore = store;
                ModelStore.EventManagerDirectory.ElementAdded.Add(new EventHandler<ElementAddedEventArgs>(StereotypeInstanceAdded));
            }
        }
      ...
      /// <param name="e"></param>
        private static void StereotypeInstanceAdded(object sender, ElementAddedEventArgs e)
        {
