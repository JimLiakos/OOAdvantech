        ...
        public static EnvDTE.DTE GetIDEInstance(string solutionFile)
        {
            ...
            if (System.IO.File.Exists(solutionFile))
            {
                try
                {
                    ...
                    System.Diagnostics.Process process = System.Diagnostics.Process.Start("devenv.exe");
                    while (ide == null)
                    {
                        foreach (System.Collections.DictionaryEntry entry in MsdevManager.Msdev.GetIDEInstances(false))
                        {
                            if ((entry.Key as string).Contains(":" + process.Id.ToString()))
                            {
                                ide = entry.Value as EnvDTE.DTE;
                                break;
                    ...
                    ide.Solution.Open(solutionFile);
                    return ide;
                }
            ...
            while (monikerEnumerator.Next(1, monikers, out numFetched) == 0)
            {
                try
                {
                    ...
                    if (!(runningObjectVal is EnvDTE.DTE))
                        runningObjectVal = RoseMetaDataRepository.RoseObjectProxy.ControlRoseObject(runningObjectVal);
